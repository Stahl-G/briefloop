...
def main():
    ...
    if not url:
        ...
        for path in [root/'pyproject.toml',*sorted((root/'src/briefloop').rglob('*')),*sorted((root/'src/wikiskill').rglob('*'))]:
        ...
        if installed!=signature:
            print('正在安装 BriefLoop…',flush=True)
            subprocess.run([str(python),'-m','pip','install',str(root)],cwd=root,check=True)
            marker.write_text(json.dumps({'signature':signature}))
        subprocess.run([str(python),'-m','briefloop','start','--workspace',str(workspace),'--port',str(args.port)]+(['--backend',args.backend] if args.backend else []),cwd=root,check=True)
        url=live_url(workspace)
