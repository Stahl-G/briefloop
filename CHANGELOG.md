...
- 行业定期报告与内置研报版式的 Word 输出新增目录页：标题写入稳定书签，目录域打开时自动更新；通用简报输出不变，导出身份号升级以刷新缓存。
- 内置版式矩阵随包分发：8 类文体（通用/商务/学术/公文红头/年报/合同/纪要/券商研报）× 5 套主题（极简蓝/商务蓝/学术黑/政务蓝红/创意橙）共 36 套；公文按 GB/T 9704 固定红头与字体只出正典一格。模板页为画廊布局：文体卡片（图标与适用说明）+ 配色点选主题，底部「已选」栏与「使用该模板」确认后设为默认并跳转新建表单；设置页下拉按文体分组。工作区首次打开自动注册；标题按章节名匹配模板样式（含序号归一），不再要求手工锚点。
