...
- 内置 13 套模板随包分发：8 类文体版式（通用/商务/学术/公文红头/年报/合同/纪要/券商研报，各带正典主题）加商务报告 5 套主题变体；工作区首次打开自动注册（封面分页、页脚页码域、标题层级、三线表、中文字体），模板页按内置/我的分组，选择器标注内置；标题按章节名匹配模板样式（含序号归一），不再要求手工锚点。
