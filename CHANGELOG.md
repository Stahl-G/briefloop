...
- 内置引擎停止主任务时取消所属子任务，已保存结果可恢复；同一对话提交回执与任务原子保存，重复提交不重复生成。等待子任务不作为模型流静默超时，取消不等待子工具自然返回。真实网页与公开周报验收发现并修复工具进度重复、Unicode 预览截断、需求额度遗漏和修订元数据晚拒绝；保留失败及恢复记录。原生桌面验收仍待完成，未作为正式发行发布。
