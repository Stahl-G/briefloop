...
- 内置版式矩阵随包分发：8 类文体（通用/商务/学术/公文红头/年报/合同/纪要/券商研报）× 5 套主题（极简蓝/商务蓝/学术黑/政务蓝红/创意橙）共 36 套；公文按 GB/T 9704 固定红头与字体只出正典一格。模板页按文体出卡片、颜色点选主题、点选即设为新建报告默认版式；设置页下拉按文体分组。工作区首次打开自动注册；标题按章节名匹配模板样式（含序号归一），不再要求手工锚点。
