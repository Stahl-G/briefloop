...
- 英文报告成为完整写作选项：语言随报告进入规划、写作、修订、评价与审阅，英文篇幅按词显示；新增10套英文版式，Word、HTML/PDF与Excel固定标签跟随报告语言。
- 新增 Excel 报表导出，可按表分工作表或整份报告单表导出。未安装 OfficeCLI 也可使用基础导出；可选增强失败或产物损坏时保留有效基础文件，支持取消、预览与去重复用。
- OfficeCLI 质检区分观察与失败，明确外部渲染服务说明；修复并发预览图片与元数据不一致、合并表头、大小写重名工作表，以及以等号开头的文本被误当公式。
- 同时支持 OpenCode 1.x 与 2.x，按实际宿主使用对应服务、会话、模型目录、推理档位和子任务协议；保留 v1，不自动迁移旧会话或认证。v2 Provider 配置在 OpenCode 中完成，已开始会话的系统约定变化需明确新建对话。
- 审阅拆分为默认普通模式与严格模式。普通模式支持 Codex、OpenCode 与 BriefLoop Agent；严格模式目前由 BriefLoop Agent 强制限定核查包和工具访问。事实与证据标准一致，实际模式和后端随任务冻结并显示，历史未知记录不冒充严格结果。
- 修复 Native 中文任务包读取和 Windows 更新缓存定位；英文模板选择、Word 渲染缓存与打包模板清单同步修正。
- 数值核验新增欧元、英镑、按数量级的股份、质量与电量单位；不同币种、功率/电量和百分比/百分点仍分开处理，未知单位不冒充已核验。
- 状态刷新始终保留正在排队或执行的旧任务，最近记录增加后不会把它们从任务列表挤掉；已结束历史仍保持有界。

本轮正式发布 macOS Apple Silicon、Python/CLI/PyPI 和 GitHub 源码；Windows 安装包保留既有稳定版，待原生验收后另行发布。Mac 包未做 Developer ID 签名或 Apple 公证。渠道状态及工件哈希以本次发行记录为准。

## 0.24.0

- 汇合 Windows 与 Native 两条开发线，延续 BriefLoop Agent / Agent CLI 统一入口。Native 的对话、研究、写稿、独立评价与修订继续使用同一份来源和版本记录；支持共用写稿装配工具，减少手动填写富文本结构。
- 报告草稿、附件和所选版本在刷新或连接恢复后保留；轮询串行处理，旧请求不能覆盖新选择。审阅页展示本次冻结的条款及旧版要求检查，重大缺口不能用混合了软硬要求的父 ID 模糊处理。
- 托管搜索与网页读取通过 SQLite 合并同一任务的并发调用，共用预算；任务阶段结束后迟到结果保留记录但不接纳为有效来源。登录和访问拦截页不会作为可用原文，正文检索支持有界分页与显式正则。
- 写稿修改在响应丢失后可重放当前回执；旧请求和已覆盖版本仍被拒绝。修订保留引用并返回当前字数，数值位置提示不冒充事实核查。
- Word 来源保留可点击链接和本稿定位，普通及模板导出绑定同一冻结来源快照；更新缓存身份，避免复用来源名称或链接已变化的旧文件。修复 DOCX 表格和空字符串单元格读取。
- Windows 修复凭据私有保存、中文任务包解码、PDF 导出取消后的恢复、更新失败重试及安装器校验。Codex 子任务活动按实际公开身份和用量展示，不推断未提供的信息。
- WikiSkill 学习保留角色绑定；内部离线试写统一关闭联网和事实核查。OpenCode 空闲回收失败时保留所属进程句柄并重试，避免不断新建后台。

下载入口和更新状态以实际发布工件为准。桌面仍需兼容 Python，首次准备依赖需要网络；签名、公证与各平台原生验收以本次发行说明为准。

## 0.23.2 · 源码交接版本

本次发布 GitHub 源码标签，供后续平台构建与验收。未发布此版本的 PyPI、macOS 或 Windows 安装包，不代表正式更新渠道已升级。

- 欢迎页与设置统一为“BriefLoop Agent / Agent CLI”，使用现有品牌图标；内置入口提示基于 Pi SDK，模型与推理强度仍可直接选择。
- 桌面启动页统一工作区卡片与原生标志，正常启动复用已准备的兼容环境；检查运行环境保留独立入口。
- 公共界面统一字号、色彩、输入框、按钮、圆角和焦点样式；修复报告顶栏标题被操作区挤压、窄屏侧栏文字与模板配色溢出。Word 保留一键下载与报告标题文件名。
- 报告分类图标优先读取本次冻结内容方法，首页与报告列表一致；清理重复新建入口，无定时计划时保留紧凑的新建入口。
- 更新 DESIGN.md 的实际实现范围，新增公共样式的令牌与构建一致性检查；历史样式仍按组件渐进迁移，不宣称全量清理完成。

- BriefLoop 内置引擎接通主链：对话与需求准备、冻结计划、并行 Scout、轮间交接、写前对照、Analyst 富文本写稿、独立评价和一次自动修订继续使用现有任务与版本系统。设置与聊天可选内置引擎，API 提供商表单可保存其本地密钥和自定义模型；不会启动其他 Agent CLI。
- 内置引擎停止主任务时取消所属子任务，已保存结果可恢复；同一对话提交回执与任务原子保存，重复提交不重复生成。等待子任务不作为模型流静默超时，取消不等待子工具自然返回。真实网页与公开周报验收发现并修复工具进度重复、Unicode 预览截断、需求额度遗漏和修订元数据晚拒绝；保留失败及恢复记录。早期候选完成部分 Mac/WPS 验收；当前集成版的跨平台发布验收尚未完成，未作为正式发行发布。

- 目标用时改为软目标，超过后继续执行；单轮硬上限移至高级“最长运行保护”，默认关闭。报告保存创建时的时间选择，修改工作区默认值不影响在途报告；连接故障检测和手动停止独立保留。
- 加强需求准备和共享 Analyst 写作方法：围绕读者问题选择比较基准、解释变化与业务影响，主体先写、摘要后写，不机械套框架；需求已足够时不追问无关基础资料。应用需求保留精确期间、独立写作偏好和人工章节，避免对象被显示成文字占位。

- 写作候选版本共用完整预检与接纳，检查后修改正文或元数据需取得新回执；兼容 CLI 支持按版本提交。字数偏好与工具未支持提示不要求改事实或凑字数，语义效果仍须真实对照验证。

## 0.23.1

- 对话与模型设置共用推理强度入口；Claude Code、CodeBuddy、Antigravity、Pi 使用原生档位，ACP 宿主按当前模型公开的选项设置，未公开能力的宿主沿用默认。报告和独立角色继承冻结的推理配置。
- Opencode 和 MiMo 改用完整下拉列表，选着 high 时仍可看到其他档位；按所选模型读取 variant，并保留自定义档位入口。
- 修复 Antigravity Gemini 模型名中的档位与显式 effort 冲突；选择默认时清除覆盖，切换 CLI 不再意外继承 Codex 的 high。
- 宿主档位读取只启动临时元数据进程，读取结束关闭；防止旧模型的异步响应覆盖新选择。

## 0.23.0

- 首页、报告、模板和设置统一使用设计令牌；分类图标共用色板，任务名、宿主名、时间格式与 HTML 转义收敛到公共定义。
- 首次使用选择宿主后，对话会同步使用该宿主及模型，不再残留初始宿主选择。
- 修复首次创建定时报告的入口被空列表隐藏，以及切入已有对话后任务栏未随任务完成收起的问题。
- Opencode 的推理档位随每次请求发送；设置应用到下一回合会同步该档位，选择“模型默认”不再回退到全局旧值。
- ZCode 默认明确使用 `build` 模式，旧 `native` 记录按 `build` 执行；只有用户明确选择 `yolo` 才关闭逐项询问，避免无界面 CLI 默认值意外扩大权限。
- 前端 API、评分面板和通用显示函数拆分为独立模块；保留报告版本、导出与恢复行为。

- 新增 ZCode 宿主：通过它的无界面 stream-json 运行，支持正文、推理、工具、用量与会话续接，权限用 ZCode 原生的 build/edit/plan/yolo。ZCode 已弃用 ACP，改用自有协议，因此走独立适配而非现有 ACP 通道。它的无界面运行没有选择模型的参数，模型沿用 ZCode 自身配置，界面如实显示该模型，指定其他模型会被拒绝；无界面下没有逐项授权通道，被权限模式拦下的操作直接失败，规划模式只提交计划、没有回答的回合记为失败。
- 报告顶栏直接下载 Word，制作完成自动下载并使用报告标题命名；其他格式提供独立 HTML 和 PDF：桌面版在后台离线渲染后直接另存为 PDF，浏览器通过打印对话框另存，都不再打开新窗口。
- 增加报告删除入口，运行中任务保护，保留来源及内部核查引用记录。
- 报告版本旁新增“查看改动”：与上一稿、第一稿或中间版本进行只读富文本比较，显示文字增删及表格、图片、格式变化；标记不会进入正文与 Word 导出。
- 报告导出菜单停留在“版式”下拉时不再意外关闭；下拉标签与选项宽度可读，含下拉的标签不再声明为菜单项。
- 内置模板构建计划、模板注册表、实际交付资产与产品文档中的后端清单、默认签名状态互相核对，数量与清单不一致会被测试拦下。
- 后台 bridge 任务遇到宿主授权请求时自动拒绝并立即失败，不再挂起到任务超时；有人在场的对话仍由用户回答。
- 报告派发不再被长时间运行的后台任务或排在队首、暂时无法开始的报告卡住；不同报告的独立审阅可以同时进行并分别停止，同一报告的审阅与核查仍按顺序执行。
- 轮询状态只列出版本摘要，不再携带所有历史正文；打开报告和比较版本时按需读取正文，报告搜索改由服务端检索正文，已删除报告不出现在结果中。
- 网页上传来源改为直接传原始文件，PDF 单文件上限提高到 100 MiB，其他文件仍为 18 MiB；PDF 页图按登记的页数读取，不再重复解析。
- DOCX 来源和 Word 模板按实际解压量限制展开大小，声明大小与实际内容不符的文件会被拒绝。
- Agent 发起的网页抓取和来源复查只访问公网地址；用户在界面上添加、重试或复查的网址仍可访问内网，但不读取本机回环与链路本地地址。
- Windows 桌面：读取网址、PDF、检测宿主版本和启动工作区时，子进程不再继承桌面的所有权输入管道。
- 桌面环境的 PATH 找不到 Node 时，仍可启动通过 npm 安装的宿主 CLI；Codex 模型列表不再显示标记为 `hide` 的内部条目。
- 在生成前核验独立审阅能力：执行后端未验证受限 Reviewer 时，事实核查在建报告、智能体提交、定时计划和排队时就被拒绝并说明可用后端，不再写完稿才失败；企业内部报告照常生成，评分明确标为普通评分，正式交付面板说明需要改用哪个后端完成独立审阅。
- 自动学习改为事前授权：新工作区默认关闭；开启自动学习或手动启动学习前，会显示历史报告数、轮数、试写次数上界、执行后端与模型，确认后才会调用模型。从旧版本升级的工作区先暂停自动学习，等待确认一次；调高轮数后暂停到再次确认。每批学习记录确认时的上限，试写次数和轮数超过上限即停止，不启用候选，反馈保留。报告选项里的“WikiSkill”开关改名为“自动学习”，已启用的技能照常用于报告。确认绑定当时看到的那份计划：确认之后改了轮数、执行后端、主模型或角色模型，自动学习会暂停到重新确认；Agent 只能把上限报给用户，不能代替用户确认；升级前排队、没有确认记录的学习批次会暂停并保留反馈。
- 内置引擎保留受限独立审阅：Reviewer 仅可读取冻结核查包、追溯主张、算术核对和提交结果，无 shell、文件写入、联网或委派工具；与主链各角色的工具权限分开。
- 独立审阅可以单独选择执行后端和模型（设置 → 独立审阅执行后端，可选 Opencode CLI 或内置引擎），与主链后端分开：用 Codex CLI、Claude Code 等尚未验证受限审阅的后端写报告时，也能开启事实核查、完成独立审阅并正式交付。选择在建任务和排队时随任务冻结，之后修改设置不影响已排队的任务。`briefloop serve --backend` 也可选择已接通主链的内置引擎。

## 0.22.3

- Word 导出合并可识别的内部来源索引，只保留面向读者的来源列表；来源标题使用可点击的原文超链接。不能可靠识别的业务表格保持原样。
- 更新通用及定制模板的 Word 缓存身份，已有报告再次导出时使用新的来源呈现；保留历史文件。

## 0.22.2

- 首页和新建报告提供紧凑研究深度、事实核查与 WikiSkill 控件；默认执行上限 60 分钟，深度研究默认目标 10,000 字、上限 12,000 字，保留用户明确篇幅。
- 同时生成报告数默认 4，可在设置中调整；Scout 并发独立调整，冻结计划不再受较低预设限制。排队等待与执行时间分别展示。
- 新报告生成期间显示独立等待页，版本选择器列出排队和生成中报告；已有稿件仍可切换查看。
- 核查与评分面板轮询保留已有内容，相同结果不重建，旧版本响应不覆盖新版本。

## 0.22.1

- 修复桌面后台启动 OpenCode 时继承桌面生命周期输入管道、导致服务启动超时的问题；OpenCode HTTP 服务使用独立的空输入，桌面退出检测和所属进程回收继续保留。
- 包含下方 0.22.0 的多渠道搜索、报告时间范围及任务进度改进。Python、CLI、Mac 和 Windows 使用同一冻结提交及同一份后端 wheel；实际渠道以发行页工件为准。

## 0.22.0

Python / CLI / Web 包已上传 PyPI；桌面候选验收发现 OpenCode 启动问题，未正式发行桌面包，由 0.22.1 修复后接续。

- 新工作区默认 Tavily 优先并允许宿主自带搜索补充；新增智谱搜索 Key 与引擎选择，博查和智谱均由用户主动启用。已有任务保持冻结搜索策略。
- 搜索支持首选、允许的补充渠道和宿主原生搜索；可主动检查跨市场覆盖或仅按缺口补查，策略随报告冻结。
- 新增博查中国市场网页搜索及本机 Key 设置，与 Tavily、DuckDuckGo 共用搜索预算和 URL 去重。
- 研究预算可查看渠道、用途、耗时及失败状态；来源页增加发现渠道筛选。移除尚无专门接入的公众号、小红书选项，不将搜索摘要标成全文或事实核实。

- 新建报告从系统时钟冻结截至日期、时间范围与时区，页面可预览并修改；定时报告每期重新计算相对时间。Tavily 检索携带冻结日期范围，写作与审阅区分当期变化与历史背景。
- 我的报告增加阶段、实际耗时、成功与失败搜索、已保存材料、研究轮次、审阅与修订进展。已有稿件可提前打开；停止、恢复和历史失败记录沿用原任务，不显示虚构完成百分比。
- Antigravity 无界面报告任务获得明确的长任务超时配置；超时或仅有部分输出时保留实际错误，避免误报成功。

使用方式见 [搜索渠道与来源](docs/使用指南.md#搜索渠道与来源)、[桌面安装与更新](docs/桌面安装与更新.md)。安装包仍未签名或公证；搜索渠道需有相应凭据或宿主能力，服务额度与费用以账号实际情况为准。

## 0.21.1

本节记录源码变更；各渠道发布状态以发行页实际工件和版本核验结果为准。

- Antigravity 支持通过原生 `view_file` 读取对话图片；工具名称、失败详情和无正文错误在对话中可见，公开错误输出统一遮蔽凭据。
- Antigravity 权限提供默认、全机访问、Turbo 与自定义预设，保留已有终端沙箱及适用的逐工具规则；无界面模式下的拒绝会明确显示，用户可调整规则后重发。
- 复制消息或来源链接时，浏览器拒绝异步剪贴板 API 后尝试本地回退，并恢复原输入焦点。
- Mac 应用内更新改用 ZIP 与 blockmap，复用已校验的同格式缓存下载差分；DMG 继续用于首次安装。差分不可用时回退到完整校验下载，首次 ZIP 更新仍可能全量下载。
- Mac 安装前检查 ZIP 路径、应用身份与目标版本，准备 Finder 中的应用替换目录。当前仍由用户替换应用并重新打开，不宣称静默安装或已签名、公证。
- 修复 OpenCode 进程退出测试的终态事件竞态，避免后台正常补写终态事件被误判成额外运行。

使用说明见 [模型与权限](docs/使用指南.md#模型与权限)及 [桌面安装与更新](docs/桌面安装与更新.md)。本版不新增事实核查能力；其真实运行的质量结论仍需独立审阅。

## 0.21.0

- 新增快速、标准和深度研究档位；深度研究预排四轮，逐轮交接发现与追问，保存已登记来源与定位。网页、对话和 external 请求将档位绑定到任务，恢复时不改用当前设置。
- 新增 DuckDuckGo 免搜索密钥入口，与 Tavily 共用受控搜索预算及失败分类。模型费用仍由所选宿主与账号承担。
- 新增可选独立事实核查：对绑定正文的公开主张补查中性与反证来源，记录范围内支持、矛盾、证据不足或未知候选；独立 Reviewer 再核对。执行状态与事实判断分开，旧结果不能冒充修改后版本的结论。
- 修正核查排队顺序：最终审阅与一次自动修订等待核查完成或明确失败，防止首轮审阅遗漏晚到结果；取消报告同时停止关联核查，失败保留草稿。
- 核查结果与阶段收束原子保存，拒绝取消后的迟到结果；定时计划保存时确定核查开关，工作区默认变化不会无感增加核查。统一核查任务名称与实际运行状态显示。
- 整合 0.20.4 的首页定时报告、预设与自定义频率及 WikiSkill 配置冻结；保留桌面差分更新。

各平台提供的版本以发行页实际工件为准。使用方法见 [深度研究与事实核查](docs/深度研究与事实核查.md)。

## 0.20.4

- 首页新增定时报告管理，支持每日、每周、每月预设及自定义间隔，显示时区与下次执行时间，可编辑、暂停、立即运行、停止本次及查看成稿。仅工作区服务运行时执行，重启跳过离线时点，同计划不重叠运行。
- 每期按执行时设置冻结宿主、模型和当前启用技能；用户反馈复用 WikiSkill 学习链，过去报告保留原技能绑定。
- Mac 更新可复用已校验的本地 DMG 缓存，按块下载并校验完整文件；无法复用或校验失败自动转全量。Windows 明确启用 NSIS 差分更新。节省比例取决于版本间实际变化，首次无缓存仍需全量。
- 下载入口和更新通知按实际发布工件提供版本。

## 0.20.3

2026-09-13 发布；安装包、PyPI 和更新入口以实际渠道提供的版本为准。

- 统一发行检查要求桌面后端与同一份发行 wheel 的 SHA-256 一致；导入的跨平台检查失败不会被相同版本号覆盖成通过。
- 纳入下方 0.20.2 的运行时卡片改进，并接入 DeepSeek Harness、Antigravity 和 Pi 的原生会话；聊天中可选择宿主实际支持的权限。模型、账户可用性及受限审阅能力仍以实际宿主为准。
- 修复桌面主进程异常退出后的后台归属清理；停止时先保存已接纳的本地修改，再取消外部 MCP 调用。工作区切换先检查目标，启动失败时提供恢复路径；桌面一次管理一个活动工作区。
- OpenCode 服务绑定启动它的 Python 进程；macOS/Linux 上父进程被强制结束后回收所属服务及进程组，Windows 沿用所属 Job 清理。不按程序名称清理用户的其他 CLI。
- OpenCode 与 Codex 后台按需启动，无活动回合、子任务或在途配置请求时空闲 30 秒回收；保留已保存的原生会话供下一回合恢复。OpenCode 模型列表缓存命中时不重新启动服务。
- 通用 CLI Bridge 空闲 30 秒释放，活动调用和会话事件订阅保留连接；退出时清理它启动的模型目录探测与执行子进程。网页空闲/隐藏时降低轮询频率，回到前台立即刷新，慢请求不重叠。
- 后台任务队列空闲时逐步降低查库频率，同服务入队后立即唤醒；其他进程提交保留最多 5 秒的发现间隔。独立 CLI 服务不会因为关闭网页或长时间无操作而自动停止。
- 修复运行时发送背压、进程死亡后会话不结束，以及子任务同名工具的新调用被进度去重遗漏的问题；无变化的心跳不重复写成进展。
- 相同 Word 输入的并发导出共用一个任务，文件缺失或损坏可重新制作。修复富文档行业报告的重复首标题、图表来源遗漏与非法图片导致保存状态无法恢复的问题。
- Markdown 导入需要明确确认转换；普通保存保留富文档的颜色、表格、图片和引用，历史版本仍可查看。上传前显示文件大小限制，超限不先读入整份附件。
- 修复软性要求的条款 ID 被误当成硬性要求；事实矛盾继续按事实问题处理。审阅发现可引用本次固定包内的来源陈述，来源陈述不会因此自动成为正文已核实的主张。
- 学习比较核对冻结的内容方法与评价条件；初始化中断可恢复，过滤后没有可用材料的案例说明原因，不记为候选技能收益。
- 损坏 PDF/XLSX 的已知解析错误定位到对应材料；连接器配置损坏时保留原文件和诊断，历史稿件仍可查看。任务结束释放所属连接器资源。
- 研究轮次变更、企业背景事实与冲突登记采用原子提交，避免并发覆盖和中断留下无法处理的记录。
- 补齐日志凭据遮蔽、连接器动态加载环境变量限制，以及私有 GET 接口的跨站来源检查；正常网页下载和本地外部客户端沿现有接口使用。
- 连接测试保留独立历史入口，错误和取消结果可查看；普通最近会话不再混入连接测试。统一聊天工具栏文字样式。
- 尚未选择模型时仍可查看和下载已有报告、读取来源及打开设置；开始新对话或新报告时再确认执行模型。

## 0.20.2 — 源码改动，未独立发行

以下改动已合并源码，将随 0.20.3 一同发行；没有单独的 0.20.2 PyPI 包或桌面安装包。

- 运行时卡片加入本地品牌图标、简介、检测版本和当前模型；图标随应用分发，离线可见。未安装运行时展开为双列卡片，窄屏自动变成单列。
- 去掉首页聊天框重复的“当前：runtime”，保留可操作的宿主选择器和回合执行记录。
- 修复 Kilo、Kiro、Vibe 已有 ACP 适配却被可选宿主名单阻挡的问题；安装检测、协议适配和实际调用结果分别呈现，不把模型目录当作账号调用成功。
- 更新失败分别显示检查失败、下载未完成、安装未完成；检查失败不附带旧版本目标，重试按失败阶段执行，保留具体错误与限流恢复时间。


## 0.20.1 — 2026-09-13

本次发布 Python / CLI / Web 包；桌面源码版本同步，未重新发行 Mac / Windows 安装包。

- 新增工作区动态与侧栏未读红点：报告任务开始、完成和失败，用户模板、Wiki 内容及可用版本更新。支持查看对应页面、全部标为已读，刷新或重开后保留已读状态，重复事件不重复提醒。

- 新增跨平台版本核对：从 pyproject.toml 读取目标版本，比较源码、实际 CLI、Mac/Windows 程序及其内置 wheel、PyPI 最新版和包哈希；缺少平台证据不算一致。
...
- 修复 WPS 表头单独留在上一页页底；内置模板导出清理旧 Courier 声明，保留用户模板字体。新版生成使用新导出指纹，历史 Word 文件保持原样。Windows WPS 的该字体提示已验证消除；Mac WPS 仍可能提示缺失字体。
- Word 表格继承模板单元格段落样式，列数与模板样例不同时按内容分配宽度，并保持单行完整，避免长指标挤窄或跨页断行。
- 修复列表结尾报告首次聚焦时自动产生空段落并误保存用户修订的问题。

- 接通会议纪要内容方法和内置纪要模板建议：依据实际转写或笔记区分讨论、决议、建议、改口与未定事项；无会议记录时阻止生成，材料提示同步说明输入要求。方法随任务保存，独立评价核对负责人和期限依据。

- 同一对话可显式选择下一回合宿主并回切，交接可见历史，保留原生会话归属和已冻结任务；详见 [对话宿主切换](docs/对话宿主切换.md)。
- 新增 WorkBuddy 等本地 Agent 使用的外部 CLI 与 Skill：只读发现已有服务、提交和查询报告、读取材料与稿件、按 base_version 保存富文档修订、导出并校验下载指定版本 Word。写请求通过稳定 request_id 重试，复用原任务和原版本。

## 0.20.0 — 2026-09-13

- 新增 Electron 桌面 App：原生工作区选择、Word 另存、保存后退出、所属服务停止与重开；提供 macOS Apple Silicon DMG 和 Windows x64 EXE。
- 精简桌面安装内容：复用 Electron 内置 Node，检测本机 Python 3.11+，使用 App 专属虚拟环境安装依赖；缺少 Python 时给出安装入口。沿用普通安装包，不新增在线安装器。
- 设置新增版本更新：检查官方发布、下载进度和用户发起安装；安装前保存编辑并停止所属任务，失败保留错误及恢复路径。
- CodeBuddy 接入原生 ACP：宿主模型目录、自定义模型 ID、对话续接、文件授权、读写与取消。受限 Reviewer 仍要求宿主实际支持。
- Codex 官方 OpenAI provider 的适用配置可选择 Fast；配置随消息、任务和角色模型保存，恢复继续沿用原配置。
- 通用报告与商业报告接通独立内容方法，包含行业周月报、工作进展周报和决策分析；实际方法版本随报告保存，Word 版式单独选择。
- MCP 材料按报告明确选择、冻结授权与预算；实际读取保存原件、回执和来源。修复旧凭据格式与可选空目录影响设置使用的问题。
- 执行时限支持“不限时”，保留手动取消。对话显示提供商错误及实际重试状态，报告等待权限时提供任务对话入口。
- 修正评价条款引用校验、行动建议被误判为既成事实的问题；区分原稿、当前修订与历史导出版本，修复保存与运行状态更新。

本版仍不提供同对话跨宿主切换、WorkBuddy 外部任务入口，或会议纪要／证券研究的专门内容流程。八类 Word 版式不等于八条内容流程。0.20.0 默认桌面安装包未签名，Mac 包也未公证；首次安装的系统提示及处理步骤见 [桌面指南](docs/桌面安装与更新.md)。

## 0.19.0 — 2026-09-12

- 首次使用可在空工作区导入合成周报，直接查看、编辑、导出；标明未调用模型、未评分，关闭自动学习，保留模型选择。并发导入和中途中断重试共用同一份示例。
- 数值检查真正限定原文行、PDF 页或结构化证据位置，不再在整份来源中借用摘录；定位不明仍显示未检查，已确认数值错配阻断正式交付。主体、期间、口径仍需审阅。
- 排队时冻结并发上限，研究计划与实际提示词使用同一快照。Node 可通过 BRIEFLOOP_NODE 指定；缺少 Node 时保留 Codex / Opencode 原生发现。
- 修复空工作区状态栏异常、编辑含引用报告时被链接空属性阻断保存，以及显式 runtime 被未选择默认模型误拒绝。
- 新增 350／500 字速览预设；统一使用指南中的研究预算和模板名称。
- PyPI 元数据补齐 README、许可证和项目链接；wheel/sdist 携带原始第三方许可，前端按实际 bundle 输入生成许可清单，缺许可构建失败。CI 检查产物漂移并运行全部前端行为检查。
- 增加产品定位、已知边界、数据流向、贡献与安全反馈说明；不把交付条件、一次学习拒绝或合成测试写成事实正确率与学习净收益保证。

- 同稿换版式导出：导出菜单可选任一就绪模板，对已保存版本按新模板重排生成 Word；正文不变，导出指纹随版式区分。
- 「已保存版本检查」新增版式检查：标题层级中断、空标题、缺表头行的表格，确定性报告、不阻断交付；检查结果同时进入独立审阅输入包。
- 行业定期报告与内置研报版式的 Word 输出新增目录页：标题写入稳定书签，提供可点击目录与缓存文本，支持更新域的阅读器可刷新页码；通用简报输出不变，导出身份号升级以刷新缓存。
- 内置版式矩阵随包分发：7 类文体（通用/商务/学术/年报/合同/纪要/券商研报）各 5 套主题（品牌绿/极简蓝/珊瑚红/石墨黑/典雅灰），加 1 套固定公文版式，共 36 套；公文按 GB/T 9704 固定红头与字体只出正典一格。模板页为画廊布局：文体卡片（图标与适用说明）+ 配色点选主题，底部「已选」栏与「使用该模板」确认后设为默认并跳转新建表单；设置页下拉按文体分组。工作区首次打开自动注册；标题按章节名匹配模板样式（含序号归一），不再要求手工锚点。
