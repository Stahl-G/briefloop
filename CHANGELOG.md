...
- 新增 BriefLoop 内置引擎（第一阶段，尚未出现在界面中）：内嵌 pi SDK，只执行受限独立审阅，会话只能通过两个限定在核查包内的工具读取材料，没有 shell、写入、联网或委派能力，也不读取本机 pi 的设置。它只能绑定到审阅任务，不能作为生成、评分、研究或学习的执行后端。模型请求长时间无响应时（例如 provider 连接中断或电脑睡眠）会取消该次请求并在同一会话里重试，已读取的材料不会丢失；回复不是纯 JSON 时的纠正回合会正常交付；引擎空闲退出后，下一轮从原对话记录继续。需要 Node 22.19+。
