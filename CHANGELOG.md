...
- 自动学习改为事前授权：新工作区默认关闭；开启自动学习或手动启动学习前，会显示历史报告数、轮数、试写次数上界、执行后端与模型，确认后才会调用模型。从旧版本升级的工作区先暂停自动学习，等待确认一次；调高轮数后暂停到再次确认。每批学习记录确认时的上限，试写次数和轮数超过上限即停止，不启用候选，反馈保留。报告选项里的“WikiSkill”开关改名为“自动学习”，已启用的技能照常用于报告。
