...
- 新增 BriefLoop 内置引擎（第一阶段，尚未出现在界面中）：内嵌 pi SDK，只执行受限独立审阅，会话的工具全部限定在核查包内：搜索、分段读取、主张追溯、算术核对和结果提交，没有 shell、写入、联网或委派能力，也不读取本机 pi 的设置。系统提示词由 BriefLoop 分层装配（共用底线、Reviewer 角色、后台模式和实际工具说明），任务提示不再沿用为 Codex/opencode 写的操作说明。审阅结果通过提交工具交回，提交时当场按结构和本次允许的 ID 校验，模型只需修正出错的字段，不必整份重写。模型支持图像输入时，报告图会直接发送给模型。它只能绑定到审阅任务，不能作为生成、评分、研究或学习的执行后端。模型请求长时间无响应时（例如 provider 连接中断或电脑睡眠）会取消该次请求并在同一会话里重试，已读取的材料不会丢失；回复不是纯 JSON 时的纠正回合会正常交付；引擎空闲退出后，下一轮从原对话记录继续。需要 Node 22.19+。
- 独立审阅可以单独选择执行后端和模型（设置 → 独立审阅执行后端，可选 Opencode CLI 或内置引擎），与主链后端分开：用 Codex CLI、Claude Code 等尚未验证受限审阅的后端写报告时，也能开启事实核查、完成独立审阅并正式交付。选择在建任务和排队时随任务冻结，之后修改设置不影响已排队的任务。`briefloop serve --backend` 不再列出只能审阅的内置引擎。
