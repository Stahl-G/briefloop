...
新增深度研究选项：任务创建入口（网页表单、对话会话与外部 CLI 请求）可选研究档位 quick / standard / deep，默认 standard；档位随任务需求保存，暂停恢复后保持，冻结研究计划时按档位展开轮次与槽位。deep 档预排多轮迭代研究——每轮生成子查询、检索核对并交接发现与追问，再针对缺口开下一轮，learnings 必须挂已登记来源与定位。任务进度显示"深度研究 第 n/N 轮"，按轮次推进不按心跳刷事件。另新增 DuckDuckGo 免密钥搜索源，无 Tavily Key 时深度研究可用，预算计量与 Tavily 一致。
# 变更记录

## 0.20.3 — 发布准备中

本节记录待发行的修复；安装包、PyPI 和更新入口以实际渠道提供的版本为准。尚未完成本版各平台安装与更新验收，不表示已统一发布。
...
新增独立事实核查（可选、观察模式）：任务开启后可显式接纳核查阶段并单独计量搜索预算（任务预留或用户追加，离线任务不可用）。独立 fact-checker 拆解选定公开主张（保留财期、单位、主体等限定词），做中性加反证查询并登记原文证据，逐条给出四态候选（口径内支持、与来源不一致、材料不足、无法判断）与未查清单，内部未公开信息不进搜索框。候选仅进审阅包与检查结果面板：须经独立 Reviewer 复核接纳，包括"矛盾"候选在内不直接创建核心冲突或阻断交付；搜索失败（如限流）、取消、预算耗尽等执行状态单独展示，不代表主张真假；核查详情不写入报告正文，改稿后需重新核查。
