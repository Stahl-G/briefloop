...
import multi_agent_brief.orchestrator.runtime_state.lifecycle as runtime_state_lifecycle
...
def test_reg_read_f02_initializer_recipe_remains_not_materialized(
    tmp_path: Path,
) -> None:
    ws = write_minimal_workspace(tmp_path / "ws")
    initialize_runtime_state(
        ...
        repo_workdir=ROOT,
        actor="cli",
        recipe="first-user-pilot",
    )
    paths = runtime_state_paths(ws)
    assert _read_json(paths["runtime_manifest"])["recipe"] == "first-user-pilot"
    before = _workspace_snapshot(ws)
...
def test_reg_read_f02_repeated_initializer_remains_not_materialized(
    tmp_path: Path,
    ...
    first_workflow = _read_json(runtime_state_paths(ws)["workflow_state"])
    monkeypatch.setattr(
        runtime_state_lifecycle,
        "utc_now",
        lambda: "2026-07-12T00:01:00+00:00",
    )
    initialize_runtime_state(
        workspace=ws,
        repo_workdir=ROOT,
    ...
    workflow = _read_json(paths["workflow_state"])
    assert manifest["created_at"] == workflow["created_at"]
    assert manifest["updated_at"] == workflow["updated_at"]
    assert manifest["runtime"] == "operator"
    assert manifest["recipe"] == "second-recipe"
    assert workflow["updated_at"] == "2026-07-12T00:01:00+00:00"
    assert workflow["stage_statuses"] == first_workflow["stage_statuses"]
    before = _workspace_snapshot(ws)
...
def test_reg_read_manifest_improvement_effect_requires_registry(
    tmp_path: Path,
) -> None:
    ws = _workspace(tmp_path, materialize=False)
    paths = runtime_state_paths(ws)
    ...
    assert "improvement" in _read_json(paths["runtime_manifest"])
    assert not paths["artifact_registry"].exists()
    before = _workspace_snapshot(ws)
...
def test_reg_read_f03_progressed_stage_missing_registry_is_degradation(
    tmp_path: Path,
) -> None:
    ...
    complete_stage_transaction(
        ...
        repo_workdir=ROOT,
        stage_id="doctor",
        reason="Doctor completed through the deterministic transaction.",
    )
    paths = runtime_state_paths(ws)
    workflow = _read_json(paths["workflow_state"])
    assert workflow["current_stage"] == "source-discovery"
    assert workflow["stage_statuses"]["doctor"]["status"] == "complete"
    assert workflow["last_completion_transaction"]["stage_id"] == "doctor"
    paths["artifact_registry"].unlink()
    before = _workspace_snapshot(ws)
...
def test_reg_read_f05_decision_transaction_missing_registry_is_degradation(
    tmp_path: Path,
) -> None:
    ws = _workspace(tmp_path, materialize=False)
    record_decision(
        ...
        stage_id="doctor",
        decision="block_run",
        reason="Block before Registry projection.",
    )
    paths = runtime_state_paths(ws)
    workflow = _read_json(paths["workflow_state"])
...
def test_reg_read_f06_progress_event_survives_forged_initial_workflow(
    tmp_path: Path,
) -> None:
    ws = _workspace(tmp_path, materialize=False)
    paths = runtime_state_paths(ws)
    initial_workflow = _read_json(paths["workflow_state"])
    record_decision(
        workspace=ws,
        repo_workdir=ROOT,
        stage_id="doctor",
        decision="block_run",
        reason="Create an authoritative progress event.",
    )
    _write_json(paths["workflow_state"], initial_workflow)
    event_records = [
        json.loads(line)
        for line in paths["event_log"].read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    assert [event["event_type"] for event in event_records] == [
        "run_initialized",
        "decision_recorded",
    ]
    assert not paths["artifact_registry"].exists()
...
def test_reg_read_f02_initialization_event_must_match_initial_projection(
    tmp_path: Path,
    case_id: str,
) -> None:
    ws = _workspace(tmp_path, materialize=False)
    ...
    if case_id == "reason":
        event["reason"] = "forged progress"
    elif case_id == "stage_binding":
        event["stage_id"] = "doctor"
    elif case_id == "extra_field":
        event["progress_marker"] = True
    elif case_id == "runtime":
        event["metadata"]["runtime"] = "forged-runtime"
    elif case_id == "missing_runtime":
        event["metadata"].pop("runtime")
    elif case_id == "extra_metadata":
        event["metadata"]["progress_marker"] = True
...
@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("improvement", {"materialized_entry_ids": []}),
        ("improvement", "malformed"),
        ("fact_layer_import", {"transaction_id": "observed-progress"}),
        ("fact_layer_import", "malformed"),
        ("claim_ledger_freeze", {"transaction_id": "observed-progress"}),
        ("claim_ledger_freeze", "malformed"),
        (
            "claim_ledger_metadata_enrichment",
            {"transaction_id": "observed-progress"},
        ),
        ("claim_ledger_metadata_enrichment", "malformed"),
        ("claim_ledger_metadata_enrichments", []),
        ("unknown_extension", {"forged": True}),
    ],
    ids=lambda value: str(value),
)
def test_reg_read_manifest_progress_or_unknown_extension_requires_registry(
    tmp_path: Path,
    field: str,
    value: Any,
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "runtime_empty",
        "recipe_type",
        "manifest_workflow_created_at_mismatch",
        "manifest_workflow_updated_at_mismatch",
    ],
)
def test_reg_read_manifest_initializer_projection_is_exact(
    tmp_path: Path,
    case_id: str,
) -> None:
    ws = _workspace(tmp_path, materialize=False)
    ...
    manifest = _read_json(paths["runtime_manifest"])
    workflow = _read_json(paths["workflow_state"])
    if case_id == "missing_base_key":
        manifest.pop("runtime_state_files")
    elif case_id == "runtime_value":
        manifest["runtime"] = "hermes"
    elif case_id == "runtime_type":
        manifest["runtime"] = 1
    elif case_id == "runtime_empty":
        manifest["runtime"] = ""
    elif case_id == "recipe_type":
        manifest["recipe"] = ["forged"]
    elif case_id == "manifest_workflow_created_at_mismatch":
        manifest["created_at"] = "2026-07-12T01:00:00+00:00"
    elif case_id == "manifest_workflow_updated_at_mismatch":
        workflow["updated_at"] = "2026-07-12T01:00:00+00:00"
        _write_json(paths["workflow_state"], workflow)
    else:  # pragma: no cover - parameter contract
        raise AssertionError(case_id)
...
def test_reg_read_matching_manifest_event_runtime_tamper_is_no_receipt_non_goal(
    tmp_path: Path,
) -> None:
    ws = _workspace(tmp_path, materialize=False)
    paths = runtime_state_paths(ws)
    manifest = _read_json(paths["runtime_manifest"])
    event = json.loads(paths["event_log"].read_text(encoding="utf-8"))
    manifest["runtime"] = "forged-but-observationally-matching"
    event["metadata"]["runtime"] = "forged-but-observationally-matching"
    _write_json(paths["runtime_manifest"], manifest)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "blocked_stage",
        "completion_binding",
        "wrong_next_decisions",
    ],
)
def test_reg_read_f04_noninitial_workflow_projection_is_degradation(
    tmp_path: Path,
    case_id: str,
) -> None:
    ws = _workspace(tmp_path, materialize=False)
    paths = runtime_state_paths(ws)
    workflow = _read_json(paths["workflow_state"])
    if case_id == "completed_stage":
        workflow["stage_statuses"]["doctor"]["status"] = "complete"
    elif case_id == "skipped_stage":
        workflow["stage_statuses"]["doctor"]["status"] = "skipped"
    elif case_id == "blocked_stage":
        workflow["stage_statuses"]["doctor"]["status"] = "blocked"
    elif case_id == "completion_binding":
        workflow["last_completion_transaction"] = {
            "transaction_id": "forged-progress",
        }
    elif case_id == "wrong_next_decisions":
        workflow["next_allowed_decisions"] = ["block_run"]
    else:  # pragma: no cover - parameter contract
        raise AssertionError(case_id)
    _write_json(paths["workflow_state"], workflow)
    before = _workspace_snapshot(ws)
