...
def test_deleted_reports_stay_out_of_body_search_and_version_views(tmp_path):
    store = Store(tmp_path)
    run, brief = published(store, '季度复盘')
    kept_run, kept_brief = published(store, '季度计划')

    assert set(store.search_briefs('季度')) == {run['id'], kept_run['id']}
    assert store.brief_view(brief['id'])['markdown'].startswith('# 季度复盘')
    store.delete_report(brief['id'])

    assert store.search_briefs('季度') == [kept_run['id']]
    with pytest.raises(ValueError, match='报告已删除'):
        store.brief_view(brief['id'])
    assert store.brief_view(kept_brief['id'])['id'] == kept_brief['id']
    # Summaries in polled state still exclude the deleted report.
    assert {b['id'] for b in store.snapshot()['briefs']} == {kept_brief['id']}
