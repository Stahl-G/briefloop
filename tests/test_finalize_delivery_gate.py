...
def test_finalize_preserves_spaced_source_marker_as_reader_clean_failure(tmp_path: Path):
    output_dir = tmp_path / "output"
    intermediate = output_dir / "intermediate"
    intermediate.mkdir(parents=True)
    (intermediate / "audited_brief.md").write_text(
        "# Brief\n\nReader content with spaced marker [source : MISSING_123456].\n",
        encoding="utf-8",
    )
    ...
    assert report["delivery_promotion"] == "skipped_reader_clean_failed"
    assert report["reader_clean"]["src_marker_count"] == 1
    assert "[source : MISSING_123456]" in report["reader_clean"]["sample_findings"][0]["text"]
    assert not (output_dir / "delivery").exists()
