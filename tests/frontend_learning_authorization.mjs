...
function context(answer){
 ...
 vm.runInContext(['roleModelText','learningPlanText','confirmLearning','setAutoLearn','learningAuthorizationNote'].map(block).join('\n'),ctx);
...
test('turning automatic learning on shows the bound and records only a confirmation',async()=>{
 ...
 assert.equal(JSON.stringify(accepted.calls),JSON.stringify([['settings',{auto_learn:true,confirm_learning_rounds:1,confirm_plan:'a'.repeat(64)}]]));
...
test('the settings note explains an upgraded workspace and a raised round count',()=>{
 ...
 ctx.state.learning_authorization={state:'plan_changed',authorized_rounds:2,plan};
 assert.match(ctx.learningAuthorizationNote(),/执行后端或模型在确认之后发生了变化/);
