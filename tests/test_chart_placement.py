...
def _brief(
    *,
    ...
    parts = ["# Brief", "", "## 市场反应与分歧", "", "DEMO 本周下跌 14.95%（行情快照）；原因无证据入账。"]
    if reaction_charts:
        parts += [
            "",
            "![DEMO Close and Volume](charts/market_data/subject-price-volume.png)",
            "![One-week Return](charts/market_data/one-week-return.png)",
        ]
    parts += ["", "## 估值", "", "DEMO 的 P/S 显著低于同业。"]
    if valuation_charts:
        parts += [
            "",
...
def test_chart_outside_its_bound_section_is_blocking() -> None:
    ...
    assert set(missing) == {"subject-price-volume", "one-week-return"}
...
def test_subject_chart_renders_event_marker_bytes() -> None:
    ...
    from multi_agent_brief.product.market_data_charts import _subject_asset
    ...
    plain = _subject_asset(security, [])
    marked = _subject_asset(security, [_Event()])
    assert plain is not None and marked is not None
    assert plain.png_bytes != marked.png_bytes
    assert marked.chart_id == "subject-price-volume"
...
def test_bound_chart_omission_with_explicit_disclosure_passes() -> None:
    ...
    disclosed = brief + (
        "\n## 覆盖缺口\n\n- subject-price-volume、one-week-return "
        "因篇幅原因未收录。\n"
    )
    assert (
        chart_placement_findings(
