...
def test_completion_projection_rejects_delivery_dir_without_finalize_report(tmp_path: Path) -> None:
    ...
    assert payload["finalize_truth"]["status"] == "missing"
    assert payload["delivery_truth"]["valid"] is False
    assert "delivery_dir_exists_without_current_finalize_report" in payload["delivery_truth"]["findings"]
...
def test_completion_projection_rejects_missing_delivery_artifact(tmp_path: Path) -> None:
    ...
    assert payload["delivery_truth"]["valid"] is False
    assert any("missing delivery artifact" in finding for finding in payload["delivery_truth"]["findings"])
    assert payload["next_allowed_action"] == "inspect_invalid_delivery_truth"


def test_completion_projection_rejects_delivery_hash_mismatch(tmp_path: Path) -> None:
    ws = _workspace(tmp_path)
    _set_workflow(ws, current_stage="finalize")
    _write_gate(ws, stage="finalize")
    _write_valid_delivery(ws)
    ...
    assert payload["delivery_truth"]["valid"] is False
    assert any("hash mismatch" in finding for finding in payload["delivery_truth"]["findings"])
    assert payload["next_allowed_action"] == "inspect_invalid_delivery_truth"


def test_completion_projection_rejects_failed_finalize(tmp_path: Path) -> None:
    ws = _workspace(tmp_path)
    _set_workflow(ws, current_stage="finalize")
    _write_gate(ws, stage="finalize")
    _write_valid_delivery(ws)
    ...
    report["status"] = "fail"
    report["reader_clean"] = {"status": "pass", "sample_findings": []}
...
def test_completion_projection_rejects_reader_clean_failure(tmp_path: Path) -> None:
    ws = _workspace(tmp_path)
    _set_workflow(ws, current_stage="finalize")
    _write_gate(ws, stage="finalize")
    _write_valid_delivery(ws)
