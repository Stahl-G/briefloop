...
def test_catalyst_calendar_requires_a_future_date() -> None:
    brief = (
        "# Brief\n\n## 摘要\n\nBody.\n\n## 市场反应与分歧\n\nTOYO 周跌。\n\n"
        "## 催化剂与投资要点\n\n关注后续政策。\n\n## 财报与估值\n\nTOYO P/S。\n"
    )
    ...
    assert [f["finding_type"] for f in findings] == [
        "catalyst_future_date_missing"
    ]

    with_dates = brief.replace(
        "关注后续政策。",
        "Section 232 措施 12月4日 生效；下次财报 2026-11-19 发布。",
    )
    assert (
        required_section_findings(
            with_dates,
...
def test_valuation_obligation_needs_core_ticker_and_multiple() -> None:
    ...
    fixed = brief.replace(
        "同业比较见数据表。",
        "TOYO 的 P/S TTM 显著低于同业，市场定价与财报表现背离。",
    )
    assert (
        required_section_findings(
            fixed,
