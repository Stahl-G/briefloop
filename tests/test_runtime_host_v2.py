...
@pytest.mark.parametrize("link_position", ["intermediate", "final"])
...
def test_invalid_bootstrap_fails_before_store_creation(tmp_path: Path) -> None:
    workspace = _workspace(tmp_path)
    config_path = workspace / "config.yaml"
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    config["controlstore_v2"].pop("run_direction")
...
@pytest.mark.parametrize(
    "conflict_kind",
    ("destination_symlink", "destination_different_bytes"),
)
