...
def test_unlimited_turn_can_finish_or_be_cancelled_after_long_wait(tmp_path, monkeypatch, cancel, phase):
    import briefloop.opencode_harness as module
    from types import SimpleNamespace
        ...
        def messages(self, owner, *, directory=None):
            return [{'info': {'id': 'reply', 'role': 'assistant', 'time': {'created': 10000,
                **({'completed': 10001} if phase == 'tool-calls' or clock[0] >= 40000 else {})},
                'finish': 'stop' if clock[0] >= 40000 else ('tool-calls' if phase == 'tool-calls' else None)},
                'parts': [{'type':'text','text':'done'}] if clock[0] >= 40000 else []}]
