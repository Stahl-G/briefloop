...
2. 对 finalize、交付、导出或分享请求：
   - `run_integrity` 为 `contaminated`、`stale_or_invalid` 或 unknown 时，
     停止这些动作，不要运行 finalize 或交付；下一步是全新 run、受控 repair
     或人工审阅。
   - `run_integrity` 为 `contaminated_repaired`（finalize-complete 写入的终态
     恢复）时，不要再次运行 finalize；仅当 `delivery_truth.valid=true` 时才可
     交付，并且永久不具备 reference 资格，否则停止交付。
