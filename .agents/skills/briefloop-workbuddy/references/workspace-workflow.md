...
8. 每次启动、CLI、角色返回或中断后运行 status 投影与 `runtime next`；
   在 repair、finalize、
   quality 摘要或交付之前只跟随 handoff/status/runtime next 的当前动作。
...
每个确定性 CLI 事务之后，向用户总结 handoff/status 投影中可见的进度。raw
...
这些字段从 `& $BriefLoop status --workspace "<workspace>" --json`（Store-native
status 投影，receipt 绑定）和
`& $BriefLoop runtime next --workspace "<workspace>"` 读取。legacy completion
projection / `workbuddy diagnose` 面已退役，不要再调用它。不要从文件
存在性检查、投影文件或叙述文字重构交付、gate、finalize 或下一步动作的真值。
恢复与下一步动作读取 `runtime next` 给出的当前动作与原因；不要
...
`package_ready=true` 只表示当前 run 的 reader package 可进入交付决策。不要说
`交付完成`、`delivered` 或 `delivery complete`，除非 status 投影报告当前 run
`delivered=true`。`terminal_state=draft_created` 表示草稿已创建，不是
delivered。
...
- 如果 `doctor` 报告任何错误，停止。展示完整 doctor 输出、工作区路径、
  ...
  不能将它改成 pass；必须修复并用同一 `$BriefLoop` 重新通过；
  中断后或会话连续性不确定时重跑 doctor。
- 恢复非终态或无效时，只执行 `runtime next` / handoff 当前动作指定的
  事务；不要从 `run_integrity` 推断恢复通道，也不要交付、导出或分享。
- 如果恢复状态为 `completed_non_reference`，不要再次运行 finalize；仅当
  status 投影报告 `package_ready=true` 时才可本地交付，并且永久不具备 reference
  资格，否则停止交付。
- 对于早期阶段的草稿工作，报告 Run Card，并只继续 handoff 允许的非交付
  工作流步骤。
- 如果 status 投影没有报告 `package_ready=true`，不要执行交付；
  如果 status 投影没有报告当前 run 的 `delivered=true`，不要声称已交付。仅当
...
WorkBuddy 主会话运行确定性 CLI 事务并再次查看 status 投影。
...
render、gate、finalize-complete、status 投影的 `package_ready=true` 与准确
`delivered`/`terminal_state`；任何 residue 都必须走确定性 repair/finalize。
