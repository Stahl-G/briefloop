...
export function connectorSettings(root, api) {
 ...
 <label>认证方式<select name="credential_type"><option value="" disabled>请选择认证方式</option><option value="bearer" selected>Bearer 令牌</option><option value="authorization">原样 Authorization（如 iFind）</option></select></label>
 ...
 function edit(record=null){
  ...
  field('credential_type').value=record?.credential_type==='unknown'?'':record?.credential_type==='authorization'?'authorization':'bearer';
  if(record)field('max_response_mb').value=record.max_response_bytes/1048576;
  find('[data-credential-note]').textContent=record?.credential_type==='unknown'?'原有凭据无法读取。请选择认证方式并重新填写凭据；已有连接记录会保留。':record?.has_credentials?'本机已保存凭据。留空保留；填写时替换该连接的凭据。'+(record.env_names?.length?' 环境变量：'+record.env_names.join('、'):''):'凭据只保存在本机，不回显已保存的值。';
 ...
 form.onsubmit=event=>{event.preventDefault();run(async()=>{
  ...
  else if(http&&field('credential_value').value){
   if(!['authorization','bearer'].includes(field('credential_type').value))throw Error('请选择新凭据的认证方式。');
   body.secrets={[field('credential_type').value==='authorization'?'authorization_header':'bearer_token']:field('credential_value').value};
  }
