...
export function scheduleUI({api,getState,refresh,notice,openReport}){ const display=(s,zone)=>s?inZone(s,zone):'—';
 ...
 function render(){
  ...
  // Keep the creation action reachable even before the first saved schedule.
  if(block){block.hidden=false;block.classList.toggle('schedule-empty',!rows.length);}
  $('schedule-list').hidden=!rows.length;
