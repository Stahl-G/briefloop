...
function updateModelLabel(){const cfg={model:$('model-select').value,reasoning_effort:$('effort-select').value};$('execution-choice').textContent='即将使用：'+modelLabel(cfg);if($('setup-model-summary'))$('setup-model-summary').textContent=modelLabel(cfg);$('generate-button').textContent='使用 '+modelLabel(cfg)+' 生成简报 →'}
...
function updateComposer(){const active=chatActive(),steering=active&&$('chat-mode').value==='steer';if(steering)$('chat-permission').value=chat.session?.runtime?.permission||'workspace-write';$('chat-permission').disabled=steering||chat.busy;$('new-session').disabled=chat.busy||chat.uploading>0;$('chat-input').readOnly=chat.busy;document.querySelectorAll('[data-chat-session]').forEach(b=>b.disabled=chat.busy||chat.uploading>0);$('chat-send').disabled=chat.busy||chat.uploading>0||!$('chat-input').value.trim();const sendLabel=chat.busy?'发送中…':active?($('chat-mode').value==='steer'?'立即补充':'排队发送'):'发送消息';$('chat-send').textContent=chat.busy?'…':'↑';$('chat-send').setAttribute('aria-label',sendLabel);$('chat-send').title=sendLabel;$('chat-stop').hidden=!active;$('chat-stop').disabled=chat.busy;$('chat-mode').disabled=!active||chat.busy;$('chat-attach').disabled=chat.busy||chat.uploading>0;$('attach-existing').disabled=chat.busy;$('chat-attach').querySelector('span').textContent=chat.uploading?'上传中…':'文件';const label=$('chat-model').selectedOptions[0]?.textContent||'Luna';$('composer-help').textContent=`Enter 发送 · Shift + Enter 换行 · ${label} / ${$('chat-effort').value}${active?' · 立即补充会调整当前工作，排队消息在本轮结束后处理':''}`}
...
function renderChat(){
 ...
 $('chat-status').textContent=`${chatStates[chat.session?.status]||'准备就绪'}${runtime?' · '+modelLabel({model:runtime.model,reasoning_effort:runtime.effort}):''}${pending?' · '+pending+' 条消息排队中':''}`;
...
for(const id of ['chat-model','chat-effort'])$(id).onchange=()=>{rememberDraft();updateComposer()};
...
function renderRoleModels(){
 ...
 $('role-model-options').innerHTML=roles.map(([role,name,label])=>{const configured=state.settings.role_models||{};const value=(role==='evaluator'?(configured.evaluator||configured.scorer||configured.assessor):configured[role])||{};return `<div class="role-model-row"><span><b>${name}</b><small>${label}</small></span><label class="visually-hidden" for="role-${role}-model">${name} 模型</label><select id="role-${role}-model" aria-label="${name} 模型" data-role-model="${role}"><option value="">继承主链</option>${[['gpt-5.6-luna','Luna'],['gpt-5.6-terra','Terra'],['gpt-5.6-sol','Sol'],['gpt-6-astra','Astra']].map(([id,title])=>`<option value="${id}" ${value.model===id?'selected':''}>${title}</option>`).join('')}</select><label class="visually-hidden" for="role-${role}-effort">${name} 推理档位</label><select id="role-${role}-effort" aria-label="${name} 推理档位" data-role-effort="${role}" ${value.model?'':'disabled'}>${['low','medium','high','xhigh','max'].map(e=>`<option ${e===(value.reasoning_effort||'high')?'selected':''}>${e}</option>`).join('')}</select></div>`}).join('');
 $('role-model-options').querySelectorAll('select').forEach(select=>select.onchange=saveRoleModels);
}
async function saveRoleModels(){
 const role_models={};for(const element of $('role-model-options').querySelectorAll('[data-role-model]')){const role=element.dataset.roleModel,effort=$(`role-${role}-effort`);effort.disabled=!element.value;if(element.value)role_models[role]={model:element.value,reasoning_effort:effort.value}}
 const selects=[...$('role-model-options').querySelectorAll('select')];selects.forEach(s=>s.disabled=true);$('role-model-status').textContent='保存中…';
 try{await api('settings',{role_models});state.settings.role_models=role_models;$('role-model-status').textContent='已保存，下一次提交时生效。'}catch(e){$('role-model-status').textContent='未保存：'+e.message;renderRoleModels()}finally{for(const element of $('role-model-options').querySelectorAll('[data-role-model]')){element.disabled=false;$(`role-${element.dataset.roleModel}-effort`).disabled=!element.value}}
}
...
function renderContext(){
 const fromEvent=[...chat.events.values()].reverse().find(e=>e.kind==='thread/tokenUsage/updated')?.data?.tokenUsage;
 const usage=chat.tokenUsage||fromEvent,last=usage?.last||{},windowSize=usage?.modelContextWindow;
