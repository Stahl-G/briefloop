...
function renderTasks(){
 ...
 box.innerHTML=tasks.length?tasks.map(j=>{
  ...
  return `<div class="task-item"><i class="task-dot ${dot}"></i><div class="task-main"><strong>${TASK_LABELS[j.kind]}</strong><small>${esc(statuses[j.status]||j.status)}${milestone}${j.error?' · '+esc(j.error):''}</small></div>${running?`<button type="button" data-task-stop="${j.id}" title="停止">■</button>`:''}${['failed','interrupted','cancelled'].includes(j.status)?`<button type="button" data-task-resume="${j.id}" title="恢复">↻</button>`:''}</div>`;
...
function render(first){
 ...
 document.querySelectorAll('[data-stop]').forEach(b=>b.onclick=()=>action(()=>api('stop',{job_id:b.dataset.stop})));document.querySelectorAll('[data-resume]').forEach(b=>b.onclick=()=>action(()=>api('resume',{job_id:b.dataset.resume})));renderTasks();
