...
function renderRuntimeDiscovery(){
 ...
 const row=r=>`<article data-runtime-card="${esc(r.id)}" class="runtime-card ${r.id===chosen?'selected':''}"><div class="runtime-card-head"><span class="runtime-monogram" aria-hidden="true">${esc(r.name.slice(0,1))}</span><div><h3>${esc(r.name)}${r.id===chosen?'<span class="runtime-selected">当前选择</span>':''}</h3><p>${esc(r.version||'版本未确认')} · ${r.available?'已检测到，可选择':'仅发现，尚未支持'}</p></div><button type="button" class="outline" data-runtime-select="${esc(r.id)}" ${r.available?'':'disabled'}>${r.id===chosen?'已选择':r.available?'选择':(r.installed?'尚未支持':'未安装')}</button>${r.available?`<button type="button" class="outline" data-runtime-test="${esc(r.id)}" ${r.id!==chosen?'disabled':''}>测试</button>`:''}</div>${r.id===chosen?`<p class="runtime-current-model">模型 <strong>${esc(state.settings.model_selection_required?'待选择':state.settings.model||'宿主默认')}</strong></p>`:''}<details><summary>安装与能力</summary><p class="runtime-path">${esc(r.path||'未安装')}</p><p>${esc(r.diagnostic||'已找到本机 CLI；可选择不代表调用通过，账号与模型须通过实际请求验证。')}</p></details></article>`;
