...
v0.9.1 之后的 mainline 还包含实验性 Evidence Span Registry 控制面：可选 span schema validation、source-pack byte binding、archive hash projection，以及 Source Appendix trace audit copy。这些是可追溯和可复验控制，不是语义支撑证明，也不是 support-sufficiency gate。
...
* [Evidence Span Registry](docs/evidence-span-registry.md)：mainline 实验性 span schema、source-pack byte binding、archive projection 和 Source Appendix trace view。它不是 Claim-Support Matrix，也不是 support-sufficiency gate。
...
v0.9.1 release 增加实验性 Atomic Claim Graph schema、coverage/type validation、Analyst/Editor contract boundary 和 reader-residue projection。它不重命名 CLI、Python 包、workspace artifacts 或实验 ID。v0.9.1 之后的 mainline 新增可选实验性 Evidence Span Registry 控制，但仍未实现 Claim-Support Matrix、semantic proof 或 support-sufficiency gate。
