...
binding、archive hash projection、Source Appendix trace audit copy，以及显式
atom-to-evidence support records。这些是可追溯和 support-record 控制，不是语义
支撑证明，也不是 support-sufficiency gate。
...
v0.9.1 release 增加实验性 Atomic Claim Graph schema、coverage/type validation、Analyst/Editor contract boundary 和 reader-residue projection。它不重命名 CLI、Python 包、workspace artifacts 或实验 ID。v0.9.1 之后的 mainline 新增可选实验性 Evidence Span Registry 控制和 Claim-Support Matrix schema / policy projection，但仍未实现 semantic proof、automatic support assessment 或 support-sufficiency gate。
