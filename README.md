...
也可以只运行 Python 工具箱命令做前置检查；真正生成简报仍由 `/generate-brief` 的外部 subagents 执行：
...
# 然后在 Claude Code / Codex / OpenCode 中运行：/generate-brief ../mabw-workspace
```
...
> **Subagent-first 生成规则：**
> - `/generate-brief <workspace>` — 外部 subagents 编排完整流程（scout → screener → claim-ledger → analyst → editor → auditor → formatter）。
> - Python CLI 是工具箱，不是 workflow runtime；`prepare` 已废弃，不会再生成简报。
>
> **finalize 交付门禁：**
> `multi-agent-brief finalize --config <workspace>/config.yaml` 用于 subagent 工作流的最终交付。
> 当 subagent 写好 `output/intermediate/audited_brief.md` 后，`finalize` 会重新生成读者版本：
...
> ⚠️ **CLI-only 模式**：适用于 `init`、`doctor`、`sources decide`、`audit`、`finalize` 等确定性工具命令；不负责生成完整简报。
...
# 生成简报请在 subagent runtime 中运行：/generate-brief my-workspace
```
...
在 Claude Code 或 Codex 中输入 `/generate-brief ../mabw-workspace`。如果需要手动前置来源规划，只运行工具命令：

```bash
multi-agent-brief sources decide --config ../mabw-workspace/config.yaml
multi-agent-brief sources decide --config ../mabw-workspace/config.yaml --merge
# 然后运行：/generate-brief ../mabw-workspace
```
