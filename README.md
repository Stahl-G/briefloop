...
一个个人本地简报工具：从来源生成简报，独立评分，直接改稿，再从修订中整理经验、改进后续任务。
...
前提是本机有 Python 3.11+；生成简报使用已安装并登录的 Codex CLI，沿用你的模型配置，不修改全局认证设置。首次安装普通依赖需要网络。
...
当前开发核验：真实 Scout、Analyst、Scorer 已完成一份合成来源简报；改稿到 Maintainer/Proposer/比较的首次真实闭环仍在进行。详见 private_planning/STATUS.md；尚未宣称产品全部完成。
