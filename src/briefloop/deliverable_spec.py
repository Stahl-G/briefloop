...
传给Scout的是具体研究任务和对应方法；传给Analyst的是同一份已保存约定、原始要求和证据索引。由你核对归类完整且没有改变用户意思，交接保留同一份约定，后续评价和修订复用。''',
        'scout': '''研究交接：使用本轮已保存的 reader_contract（由任务包或 plan.json 提供），按其中的内容问题与研究方法寻找并核对证据，返回事实、支持范围、来源定位与具体缺口。随读随记证据；若工具提供 record_evidence，以 source_id/source_hash、单一行段 locator、短逐字 quote 交给运行器截取原文，只修正未接纳条目，最后仅提交缺口与检索小结；否则在槽位结果中增量保留逐字 excerpt。每条证据是一段连续原文：excerpt 逐字摘录（同一处的几段可用“…”连接），概括、换算与判断写进 facts；locator 用 line 12-18、page 3 或序列化的证据定位 JSON，只指向这段摘录；同一来源的不同段落各写一条。方法限制、抓取失败和研究状态进入结构化研究结果，供主Agent安排补查或调整判断；它们不是要求Analyst复制到正文的段落。''',
