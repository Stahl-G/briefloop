...
传给Scout的是具体研究任务和对应方法；传给Analyst的是同一份已保存约定、原始要求和证据索引。由你核对归类完整且没有改变用户意思，交接保留同一份约定，后续评价和修订复用。''',
        'scout': '''研究交接：读取 plan.json 中本轮已保存的 reader_contract（不是本文件自身），按其中的内容问题与研究方法寻找并核对证据，返回事实、支持范围、来源定位与具体缺口。每条来源的 excerpt 是原文逐字摘录（同一处的几段可用“…”连接），概括、换算与判断写进 facts；locator 用 line 12-18、page 3 或序列化的证据定位 JSON，指向摘录所在位置。方法限制、抓取失败和研究状态进入结构化研究结果，供主Agent安排补查或调整判断；它们不是要求Analyst复制到正文的段落。''',
