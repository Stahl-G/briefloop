...
def workspace_action(store, request):
    ...
    if action=='revise_document':
        ...
        return store.revise(request['base_version'],editor_document=json.loads(path.read_text(encoding='utf-8')),citations=request.get('citations'),author='agent')
    if action=='workflows':
...
- {{"action":"read_report","version_id":"稿件ID"}}：读取富文档 JSON 和引用。用户明确要求修改内容/章节/图表时，将修改后的 JSON 保存到工作区文件，再用 {{"action":"revise_document","base_version":"刚读取版本ID","document_file":"工作区内JSON绝对路径"}} 保存新版本，不覆盖用户并发编辑。可选 citations 完整替换引用列表（source_id/locator/excerpt，schema 见 capabilities），省略则保留；只修改引用也保存新版本，定位描述不表示已独立核验。
