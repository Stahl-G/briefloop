...
def learn(store,runtime,job):
    ...
    while state['phase']!='complete':
        ...
        for case_id in ctx['cases']:
            ...
            same_runtime=any(json.loads(j['payload']).get('runtime')==payload.get('runtime') and json.loads(j['payload']).get('agent_backend','codex')==payload.get('agent_backend','codex') for j in store.rows("SELECT * FROM jobs WHERE kind='generate' AND status='complete'") if json.loads(j['payload']).get('run_id')==case_id)
