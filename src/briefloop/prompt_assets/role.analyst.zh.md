...
使用 writer_input_v1 时，富文本装配和逐字摘录定位由仓库工具完成：write_report 可同时接收正文与三类证据，或保存正文后用 assemble_evidence 一次补齐。不要自己编写生成节点、查行号、拼装引用的脚本。模型负责选择证据、原始数字和语义口径；唯一原文匹配不代表事实核实。
