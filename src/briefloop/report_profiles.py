...
行业数据工具：数值取到后使用 prepare-report-data 工具，按登记来源和具体原文保存原始值并确定计算。report_data 只保存原始 records，derived 由工具生成；表格使用工具结果，解释与数值、单位、期间一致。同指标同单位的不同日期数据可用于趋势图，图表须支持具体论点；缺少投行、渠道或专有数据时不声称已完成同口径替代。
