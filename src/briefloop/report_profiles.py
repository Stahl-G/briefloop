...
允许 Analyst 在初次证据合并后提出会改变核心结论的具体缺口（缺什么、为什么重要、需要何种原始来源）；Orchestrator 仅在剩余共享预算/时间内派已有 Scout 定向补查，复用原文，默认最多一次补查交接。预算不足或无法取得时交付已有报告，把待补研究列入独立核查记录，不新增必经角色，不无限研究。
独立核查记录 research_notes/gaps 保存缺指标或证据、最近可得截至日、对结论影响和建议补充来源。正文无需重复内部核查过程；人工填写章节按指定占位，不作为研究遗漏。不强求8–10张表，不把缺数据写成没有变化，不因局部缺失挡住整份报告。''',
