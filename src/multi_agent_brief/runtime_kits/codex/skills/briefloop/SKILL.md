...
Use when operating this workspace through the SQLite-only BriefLoop Codex
runtime. The packaged kit supports `runtime invocation-start` and
`runtime apply`; never fall back to a legacy runtime.
...
For a business workspace, read `references/controlstore-v2.md` completely
...
Follow `references/controlstore-v2.md` as an executable state machine.
