...
INSTALL_MARKER = "<!-- Generated by briefloop claude install. -->"
LEGACY_INSTALL_MARKER = "<!-- Generated by multi-agent-brief claude install. -->"


def register(subparsers: argparse._SubParsersAction) -> None:
    claude_parser = subparsers.add_parser(
        "claude",
        help="Install or inspect Claude Code assets for BriefLoop.",
    )
    actions = claude_parser.add_subparsers(dest="claude_action", required=True)

    install = actions.add_parser(
        "install",
        help="Install BriefLoop Claude Code assets into ~/.claude.",
    )
    install.add_argument(
        "--repo-workdir",
        default=".",
        help="BriefLoop source repository root (default: current directory).",
    )
    install.add_argument(
        "--target",
    ...
    install.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing non-generated files in target locations.",
    )
    install.add_argument(
        "--dry-run",
        action="store_true",
...
def _install(args: argparse.Namespace) -> int:
    ...
    try:
        planned = apply_planned_writes(
            ...
            generated_markers=(INSTALL_MARKER, LEGACY_INSTALL_MARKER),
        )
    ...
    print("")
    print("[claude install] Installed /briefloop and compatibility Claude Code assets.")
    print("[claude install] In Claude Code, type '/' and confirm /briefloop is listed.")
    return 0
