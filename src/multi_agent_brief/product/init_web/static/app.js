...
(function () {
    ...
    var MESSAGES = {
        zh: {
            ...
            note_search_topic: "填写一个简短、明确的行业或主题；它会与公司 / 组织名称组合成唯一 Tavily 检索词。不要粘贴完整任务说明。",
            sec_audience: "谁会读它？",
            ...
            web_search_note: "唯一 Tavily 检索词不是完整任务目标；系统按已确认的「公司 / 组织 + 一个空格 + Human 明确填写的检索主题」确定性生成并在初始化时冻结，max_results 固定为 5。报告类型只影响报告 focus 与呈现。每个 Human/Store 授权 attempt 最多调用一次并冻结安全的原始响应及 provider 返回的 durable content。失败不会自动重试。可选域名只限制 provider 检索范围，不代表证据支持或权威性。Key 写入新工作区的私有 .env，并保留到 Human 显式轮换或移除；不进入 Store、日志或报告。",
        ...
        en: {
            ...
            note_search_topic: "Enter one concise, explicit industry or topic. It is combined with the company / organization name to form the sole Tavily query. Do not paste the full task objective.",
            ...
            web_search_note: "The sole Tavily query is not the full task objective. It is deterministically projected from the confirmed company / organization + one space + the Human-entered search topic and frozen at initialization, with max_results fixed at 5. Report type affects brief focus and presentation only. Each Human/Store-authorized attempt makes at most one call and freezes the safe raw response plus provider-returned durable content. Failures never auto-retry. Optional domains constrain provider search only; they do not prove evidence support or authority. The key is written to the new workspace's private .env and remains until the Human rotates or removes it; it never enters the Store, logs, or report.",
    ...
    var CATALOG = {
        ...
        sources: [
            { id: "public_web", zh: ["公开网页（Tavily 运行时获取）", "明确填写检索主题；每个授权 attempt 至多一次请求"], en: ["Public web (Tavily runtime acquisition)", "Enter an explicit search topic; at most one request per authorized attempt"] },
