...
(function () {
    ...
    var MESSAGES = {
        zh: {
            ...
            web_search_note: "唯一 Tavily 检索词不是完整任务目标；系统按已确认的「公司 / 组织 + 一个空格 + 报告类型 / 主题」确定性生成并在初始化时冻结，max_results 固定为 5。每个 Human/Store 授权 attempt 最多调用一次并冻结安全的原始响应及 provider 返回的 durable content。失败不会自动重试。可选域名只限制 provider 检索范围，不代表证据支持或权威性。Key 写入新工作区的私有 .env，并保留到 Human 显式轮换或移除；不进入 Store、日志或报告。",
    ...
    var CATALOG = {
        ...
        sources: [
            { id: "public_web", zh: ["公开网页（Tavily 运行时获取）", "确认检索词基底（主体 + 主题）；每个授权 attempt 至多一次请求"], en: ["Public web (Tavily runtime acquisition)", "Confirm the query basis (subject + theme); at most one request per authorized attempt"] },
