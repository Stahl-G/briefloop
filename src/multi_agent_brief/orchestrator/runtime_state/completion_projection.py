...
This module is core runtime-state code, not a WorkBuddy adapter. Runtime
adapters may format this payload, but they must not re-infer delivery truth from
file existence.
"""
...
def _finalize_truth(workspace: Path) -> dict[str, Any]:
    ...
    truth.update({
        ...
        "delivery_promotion": payload.get("delivery_promotion") if isinstance(payload.get("delivery_promotion"), Mapping) else {},
...
def _delivery_truth(
    ...
    *,
    ...
    findings = _finalize_report_delivery_artifact_reasons(workspace, dict(report))
    truth["artifact_count"] = (
        len(report.get("delivery_artifacts"))
        if isinstance(report.get("delivery_artifacts"), list)
        else 0
    )
    if findings:
        truth["findings"].extend(findings)
        return truth
    truth.update({
        "valid": True,
        "status": "valid",
