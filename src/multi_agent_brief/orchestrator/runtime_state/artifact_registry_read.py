...
from multi_agent_brief.orchestrator.runtime_state.manifest import _runtime_manifest
...
_INITIALIZATION_EVENT_FIELDS = frozenset(
    {
        "schema_version",
        "event_id",
        "run_id",
...
def _interpret_artifact_registry(
    *,
    ...
    if registry is None:
        if _is_legal_initialized_registry_absence(prerequisites):
            return RegistryNotMaterialized()
        return RegistryDegradation("artifact_registry_recovery_context_invalid")
...
def _validate_registry_prerequisites(
    *,
    ...
    stage_ids = tuple(str(stage["stage_id"]) for stage in stages)
    if not _json_values_equal(stage_ids, tuple(context.stage_ids)):
        return RegistryDegradation("artifact_registry_contract_context_invalid")
    ...
    completion_stages = [{"stage_id": stage_id} for stage_id in context.stage_ids]
    try:
...
def _is_legal_initialized_registry_absence(
    prerequisites: _RegistryPrerequisites,
) -> bool:
    """Recognize only the complete observed Registry pre-projection state."""
    ...
    if not (
        recovery_state.get("status") == RECOVERY_NOT_APPLICABLE
        and recovery_state.get("reason_code") == "no_current_contamination"
    ):
        return False
    ...
    if not (
        _valid_writer_timestamp(manifest_created_at)
        and _valid_writer_timestamp(manifest_updated_at)
        and _valid_writer_timestamp(workflow_created_at)
        and _valid_writer_timestamp(workflow_updated_at)
        and _json_values_equal(manifest_created_at, workflow_created_at)
        and _json_values_equal(manifest_updated_at, workflow_updated_at)
    ):
        return False
    ...
    expected_manifest = _runtime_manifest(
        ...
        created_at=cast(str, manifest_created_at),
        updated_at=cast(str, manifest_updated_at),
        runtime=runtime,
        stages=list(prerequisites.stages),
    ...
    if "recipe" in manifest:
        ...
        if not isinstance(recipe, str):
            return False
        expected_manifest["recipe"] = recipe
    if not _json_values_equal(manifest, expected_manifest):
        return False

    # A repeated initializer invocation advances only the Workflow's top-level
    # updated_at. Initial per-stage timestamps remain bound to created_at.
    expected_workflow = _initial_workflow_state(
        run_id=prerequisites.run_id,
        stages=list(prerequisites.stages),
        created_at=cast(str, workflow_created_at),
        updated_at=cast(str, workflow_created_at),
    )
    expected_workflow["updated_at"] = cast(str, workflow_updated_at)
    if not _json_values_equal(workflow, expected_workflow):
        return False

    event_records = tuple(context.event_records)
    if len(event_records) != 1:
        return False
    initialization_event = event_records[0]
    metadata = initialization_event.get("metadata")
    return (
        set(initialization_event) == _INITIALIZATION_EVENT_FIELDS
        and initialization_event.get("event_type") == "run_initialized"
        and _validated_run_id(initialization_event.get("run_id"))
        == prerequisites.run_id
        and _valid_writer_timestamp(initialization_event.get("created_at"))
        and initialization_event.get("stage_id") is None
        and initialization_event.get("artifact_id") is None
        and initialization_event.get("decision") is None
        and initialization_event.get("reason") == "Runtime state initialized."
        and _json_values_equal(metadata, {"runtime": runtime})
    )
