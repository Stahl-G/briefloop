"""Runtime handoff domain model and artifact writer.

This module builds handoff payloads and writes runtime handoff/control files.
CLI modules should dispatch to these helpers rather than owning the domain
logic.
"""
...
REPAIR_GUIDANCE_NOTE = (
    ...
    "For current quality-gate owner-stage artifact repair, run `briefloop gates show "
    "--workspace <workspace> --json` and follow its required_commands. Current-gate "
    "repair start must be scoped with `--gate-stage` and `--gate-artifact`; do not use "
    "unscoped repair start for current-gate blockers because it may inspect stale "
    "workspace-wide reports. For non-gate owner-stage repair routes from audit_report, "
    "finalize_report, artifact_registry, or transaction_integrity, run `briefloop repair "
    "route --workspace <workspace> --json`, then start the selected route with "
    "`--finding-id <finding_id>` or `--route-index <route_index>`; do not use bare "
    "`repair start --workspace <workspace>`. Delegate only the repair_owner role and allow edits only "
    "to allowed_artifacts. After the owner edits, run `briefloop repair complete "
...
ORCHESTRATOR_CONTEXT_READING_NOTE = (
...
FINALIZE_GATE_NOTE = (
    ...
    "there are blocking findings, do not finalize. Run `briefloop gates show --workspace "
    "<workspace> --json` and follow its scoped repair required_commands, or choose "
    ...
    "repair required_commands or choose request_human_review/block_run."
)
...
def _codebuddy_handoff(workspace: Path, repo: Path, venv: str) -> AgentHandoff:
    ...
    role_sequence = "\n".join(
        [
            ...
            "3. Default topology: Scout already writes `output/intermediate/screened_candidates.json`; do not invoke `briefloop-screener` or complete screener separately.",
            "4. Strict topology only: ask `briefloop-screener` to draft `output/intermediate/screened_candidates.json`, then run deterministic BriefLoop validation and screener stage transactions in the main CodeBuddy session.",
            "5. Ask `briefloop-claim-ledger` to draft `output/intermediate/claim_drafts.json` from screened candidates.",
            f"6. Run `briefloop state freeze-claim-ledger --workspace {ws_path}` and `briefloop state stage-complete --workspace {ws_path} --stage claim-ledger --reason \"Claim Ledger was frozen from claim drafts.\"` in the main CodeBuddy session.",
            "7. Ask `briefloop-analyst` to draft Analyst-owned artifacts from frozen `output/intermediate/claim_ledger.json`.",
            "8. Run deterministic BriefLoop validation and stage transactions in the main CodeBuddy session.",
            "9. Ask `briefloop-editor` to polish the audited brief without adding facts.",
            "10. Run deterministic BriefLoop validation and stage transactions in the main CodeBuddy session.",
    ...
    return AgentHandoff(
        ...
        prompt=(
            ...
            "Before any role work or fallback, confirm that this main session can invoke `briefloop` or `multi-agent-brief`. "
            "If it cannot, stop before role work or state advancement. Regenerating an operator handoff does not supply missing command execution and is not a fallback for a missing CLI.\n\n"
            "CodeBuddy role sub-agents are project-level assets:\n"
            f"{role_agents}\n\n"
            "In a CodeBuddy or compatible WorkBuddy host that loaded the project assets, explicitly invoke the matching BriefLoop role sub-agent by exact name for role-owned artifact work. "
            "Do not perform Scout, Screener, Claim Ledger, Analyst, Editor, Auditor, or Formatter work in the main conversation. "
            "Drafting role sub-agents declare Read, Write, Grep, Glob; the read-only Formatter declares Read, Grep, Glob. All intentionally omit Bash so deterministic CLI transactions remain in the main session. "
            "If the main session can run BriefLoop CLI commands but the host cannot dispatch the exact checked-in BriefLoop role, stop before role-owned draft work and ask the user either to continue in a CodeBuddy/WorkBuddy host with project-role dispatch or to regenerate an operator handoff. "
            ...
            "The main CodeBuddy/WorkBuddy session must expose command execution capable of invoking BriefLoop; that host capability is separate from each role sub-agent's tool allowlist. "
...
def _apply_improvement_memory_handoff(handoff: AgentHandoff, snapshot_result: Any) -> None:
