    ...
    if "weekly" in resolved_cadences:
        jobs.append(HermesCronJob(
            ...
            prompt=(
                ...
                "Run doctor, then run the deterministic prepare pipeline. If the quality or audit gate fails, "
                "do not claim delivery success; report the blocking findings and artifact paths. "
                "If prepare passes, run finalize so reader-facing Markdown/DOCX are refreshed."
            ),
        ))
    ...
    if "monthly" in resolved_cadences:
        jobs.append(HermesCronJob(
            ...
            prompt=(
                ...
                "Favor month-level patterns over daily noise. Run doctor, prepare, and finalize. "
                "If the quality or audit gate fails, report the blocking findings and do not mark the brief delivery-ready."
            ),
        ))
