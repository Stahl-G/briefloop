"""Runtime error vocabulary: RuntimeStateError and its error-code constants.

Single source of truth (LD2-2b relocation). The legacy
``orchestrator.runtime_state.errors`` path is a re-export shim until LD2-3.

Distinct from :mod:`multi_agent_brief.contracts.errors`, which carries the
contract *validation* vocabulary (ContractError / FieldViolation).
"""
