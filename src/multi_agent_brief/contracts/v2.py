...
from typing import Annotated, Any, ClassVar, Literal, Mapping, Optional, Union
...
CleanText = Annotated[str, AfterValidator(_clean_text)]
...
WorkspacePath = Annotated[str, AfterValidator(_workspace_path)]
...
SourceLocator = Annotated[Union[WebSourceLocator, FileSourceLocator], Field(discriminator="kind")]
...
class RunIdentity(StrictModel):
    ...
    runtime: Literal["hermes", "claude", "opencode", "codex", "codebuddy", "operator"]
    created_at: IsoDateTime
...
class ArtifactRecord(StrictModel):
    ...
    status: Literal["expected", "missing", "valid", "invalid", "stale"]
    required: bool
    path: WorkspacePath
    format: Literal["json", "yaml", "markdown", "html", "docx", "pdf"]
...
class EventEnvelope(StrictModel):
    ...
    actor_kind: Literal["transaction", "human", "adapter"]
    actor_id: ContractId
...
class Approval(StrictModel):
    ...
    run_id: ContractId
    subject_type: Literal["run", "artifact", "delivery", "repair"]
    subject_id: ContractId
    decision: Literal["approve", "reject"]
    decided_by: ContractId
    decided_at: IsoDateTime
    rationale: Optional[CleanText] = None
...
class Delivery(StrictModel):
    ...
    artifact_id: ContractId
    artifact_revision: PositiveInt
    status: Literal["candidate", "approved", "delivered", "rejected"]
    target: Literal["local", "feishu"]
    created_at: IsoDateTime
    delivered_at: Optional[IsoDateTime] = None
...
class TransactionReceipt(StrictModel):
    ...
    changed_record_ids: list[ContractId] = Field(default_factory=list)

    @model_validator(mode="after")
    def ordered_timestamps(self) -> "TransactionReceipt":
...
ArtifactSubmitRequest.full_example = deepcopy(ArtifactSubmitRequest.minimal_example)
...
StageState.full_example = {**StageState.minimal_example, "status": "completed", "revision": 1}
...
ArtifactRecord.full_example = {**ArtifactRecord.minimal_example, "current_revision": 1, "status": "valid"}
...
EventEnvelope.minimal_example = {
    ...
    "sequence": 1,
    "occurred_at": _NOW,
    "actor_kind": "transaction",
    "actor_id": "stage-complete",
}
EventEnvelope.full_example = {**EventEnvelope.minimal_example, "payload": {"stage_id": "scout"}}

Invocation.minimal_example = {
    "schema_version": Invocation.schema_id,
    "invocation_id": "INV-001",
    "run_id": _RUN,
    "command": "state.stage-complete",
    "arguments": ["--stage", "scout"],
    "requested_by": "operator",
    "requested_at": _NOW,
    "idempotency_key": "IDEMP-001",
}
Invocation.full_example = deepcopy(Invocation.minimal_example)
...
Approval.full_example = {**Approval.minimal_example, "rationale": "Approved after reader review."}
...
Delivery.full_example = {**Delivery.minimal_example, "status": "delivered", "delivered_at": _NOW}
...
TransactionReceipt.full_example = {
    **TransactionReceipt.minimal_example,
    "event_ids": ["EVT-001"],
    "changed_record_ids": ["STAGE-scout"],
}
...
V2_CONTRACT_IDS: tuple[str, ...] = tuple(model.schema_id for model in V2_CONTRACT_MODELS)

for _contract_model in V2_CONTRACT_MODELS:
    SchemaRegistry.register(_contract_model)


LEGACY_READ_ONLY_CONTRACTS: Mapping[str, str] = MappingProxyType(
    {
        "source_item": SourceProposal.schema_id,
        "candidate_claims": CandidateClaimsProposal.schema_id,
        "screened_candidates": ScreenedCandidatesProposal.schema_id,
        "claim_drafts": ClaimDraftsProposal.schema_id,
        "audit_report": AuditProposal.schema_id,
        "artifact_submit_request": ArtifactSubmitRequest.schema_id,
        "runtime_manifest": RunIdentity.schema_id,
        "workflow_state": StageState.schema_id,
        "artifact_registry_record": ArtifactRecord.schema_id,
        "artifact_revision": ArtifactRevision.schema_id,
        "event_log_event": EventEnvelope.schema_id,
        "runtime_invocation": Invocation.schema_id,
        "human_approval": Approval.schema_id,
        "delivery_record": Delivery.schema_id,
        "transaction_receipt": TransactionReceipt.schema_id,
    }
)
...
class ContractReadResult:
    """Read classification that never upgrades legacy payloads into v2 writes."""

    classification: Literal["canonical_v2", "legacy_read_only", "invalid"]
    requested_schema_id: str
    canonical_schema_id: Optional[str]
    ...
    def can_write(self) -> bool:
        return self.classification == "canonical_v2" and self.canonical_model is not None
...
def read_contract_payload(schema_id: str, payload: Any) -> ContractReadResult:
    ...
    A legacy classification is read-only by construction: it returns no v2
    model and exposes no migration or write method.
    """

    legacy_target = LEGACY_READ_ONLY_CONTRACTS.get(schema_id)
    if legacy_target is not None:
        try:
            ...
            return ContractReadResult(
                ...
                canonical_schema_id=legacy_target,
                violations=(FieldViolation(field="$", error="must contain JSON-compatible values"),),
            )
        return ContractReadResult(
    ...
    if contract not in V2_CONTRACT_MODELS:
        return ContractReadResult(
            ...
            violations=(FieldViolation(field="schema_id", error="unknown v2 contract"),),
        )
    try:
