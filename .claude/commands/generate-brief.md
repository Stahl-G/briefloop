...
BriefLoop uses an external subagent workflow. Python CLI commands provide setup, source planning, validation, audit checks, and final rendering tools.
...
A stage is complete only after `briefloop state stage-complete` succeeds.

After each stage produces its expected artifact:

1. Verify the expected artifact exists at the declared path.
2. Run:
   `briefloop state stage-complete --workspace $ARGUMENTS --stage <stage_id> --reason "<reason>"`
...
1. Initialize runtime handoff/control context:
   - Run `briefloop run --workspace $ARGUMENTS --runtime claude --skip-doctor`.
...
3. **Doctor gate:**
   ...
   - Run `briefloop state stage-complete --workspace $ARGUMENTS --stage doctor --reason "Doctor readiness checks passed."`.
...
4. **Source discovery transaction (all source profiles):**
   ...
   - Run `briefloop state stage-complete --workspace $ARGUMENTS --stage source-discovery --reason "Source discovery source surface was resolved."`.
   - If the transaction fails, stop and report the failure. Do not invoke the next specialist or tool.
...
5. **Input governance gate (if available):**
   ...
   - Run `briefloop state stage-complete --workspace $ARGUMENTS --stage input-governance --reason "Input governance classified reader inputs."`.
   - If the transaction fails, stop and report the failure. Do not invoke the next specialist or tool.
...
6. Read `configs/policy_packs/default.yaml` and invoke the **scout** subagent with the active role topology:
   ...
   - Check the expected artifact.
   - Run `briefloop state stage-complete --workspace $ARGUMENTS --stage scout --reason "Candidate claims were extracted."`.
...
7. Strict topology only: invoke the **screener** subagent:
   ...
   - Check the expected artifact.
   - Run `briefloop state stage-complete --workspace $ARGUMENTS --stage screener --reason "Candidate claims were screened and ranked."`.
   - If the transaction fails, stop and report the failure. Do not invoke the next specialist.
...
8. Invoke the **claim-ledger** subagent:
   ...
   - Run `briefloop state stage-complete --workspace $ARGUMENTS --stage claim-ledger --reason "Claim Ledger was built from screened candidates."`.
   - If the transaction fails, stop and report the failure. Do not invoke the next specialist.
...
10. Invoke the **analyst** subagent:
   ...
   - Check the expected artifact.
   - Run `briefloop state stage-complete --workspace $ARGUMENTS --stage analyst --reason "Auditable brief was drafted from the Claim Ledger."`.
   - If the transaction fails, stop and report the failure. Do not invoke the next specialist.
...
11. Invoke the **editor** subagent:
    ...
    - Check the expected artifact.
    - Run `briefloop state stage-complete --workspace $ARGUMENTS --stage editor --reason "Auditable brief was edited without changing evidence."`.
    - If the transaction fails, stop and report the failure. Do not invoke the next specialist.
...
16. Final response:
    ...
    - Report success only when the Store-native status projection (briefloop status --workspace $ARGUMENTS --json) reports delivered=true for the current run; audit status or artifact existence alone is not a delivery claim.
