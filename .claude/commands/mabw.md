...
If state is blocked, stop. Inspect `$BRIEFLOOP_CLI gates show --workspace
...
findings inspect `$BRIEFLOOP_CLI repair route --workspace <workspace> --json`.
...
```bash
$BRIEFLOOP_CLI workbuddy diagnose --workspace <workspace> --json
```
