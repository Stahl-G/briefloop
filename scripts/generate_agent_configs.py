...
Default generation intentionally skips human-facing prompt entrypoints:

- AGENTS.md
- .agents/skills/*/SKILL.md

Those files are hand-maintained by default because they are product/runtime
prompts, not simple platform config projections.
...
Manual prompt targets are protected. To regenerate them intentionally:
    python scripts/generate_agent_configs.py --target agents_md --write --allow-prompt-overwrite
    python scripts/generate_agent_configs.py --target skills --write --allow-prompt-overwrite
"""
...
# Default generation is limited to platform adapter/config docs.
# Human-facing prompt entrypoints are hand-maintained by default.
DEFAULT_TARGETS = {"codex", "claude", "docs", "opencode"}
HAND_MAINTAINED_PROMPT_TARGETS = {"agents_md", "skills"}
ALL_TARGETS = DEFAULT_TARGETS | HAND_MAINTAINED_PROMPT_TARGETS
...
def render_agents_md(manifest: dict) -> str:
    """Render a short runtime-neutral AGENTS.md.

    AGENTS.md is a route sign, not the full project manual.
    Role details live in platform agent files, skill files, and docs.
    """
    _ = manifest  # reserved for future version/project metadata
...
def render_skill(role_name: str, role: dict, manifest: dict) -> str:
    """Render a compact skill scaffold.

    Skill files are hand-maintained by default. This renderer is kept for
    explicit manual regeneration only via:
        --target skills --allow-prompt-overwrite
    """
    _ = manifest
    resp = "\n".join(f"- {r}" for r in role["responsibilities"])
    title = role_name.replace("-", " ").title()
    ...
    return (
        ...
        f"## Inputs\n"
        f"\n"
        f"Use the role-specific workspace artifacts documented in the hand-maintained skill contract.\n"
        f"\n"
        f"## Outputs\n"
        f"\n"
        f"Write only the artifact owned by this role and hand off to the next role.\n"
        f"\n"
        f"## Handoff\n"
        f"\n"
        f"Return the next expected artifact path, status, and any limitations.\n"
    )
...
def generate_all(manifest: dict, check: bool = False, target: str | None = None, *, allow_prompt_overwrite: bool = False) -> bool:
    validate_manifest(manifest)
    roles = manifest["roles"]
    profiles = manifest["tool_profiles"]
    ok = True

    targets = set(DEFAULT_TARGETS) if target is None else {target}

    protected_targets = targets & HAND_MAINTAINED_PROMPT_TARGETS
    if protected_targets and not allow_prompt_overwrite:
        protected = ", ".join(sorted(protected_targets))
        raise RuntimeError(
            f"Refusing to overwrite hand-maintained prompt target(s): {protected}. "
            "Use --allow-prompt-overwrite for an intentional one-off regeneration."
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Generate platform-specific agent adapter files from agent_roles.yaml. AGENTS.md and skills are hand-maintained by default.",
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--write", action="store_true", help="Generate default platform adapter files")
    group.add_argument("--check", action="store_true", help="Check if default platform adapter files are up to date")
    parser.add_argument("--target", choices=sorted(ALL_TARGETS),
                        help="Generate only a specific target. agents_md and skills require --allow-prompt-overwrite.")
    parser.add_argument("--manifest", type=Path, default=MANIFEST_PATH,
                        help="Path to agent_roles.yaml")
    parser.add_argument("--allow-prompt-overwrite", action="store_true",
                        help="Allow explicit regeneration of hand-maintained prompt targets: agents_md or skills.")
    ...
    try:
        ok = generate_all(
            ...
            check=args.check,
            target=args.target,
            allow_prompt_overwrite=args.allow_prompt_overwrite,
        )
