...
def render_agents_md(manifest: dict) -> str:
    ...
    return (
        ...
        f"# AGENTS.md\n"
        f"\n"
        f"Python commands are tools, not the workflow runtime. There is no Python brief-generation pipeline.\n"
        f"The runtime is the external subagent workflow: scout -> screener -> claim-ledger -> analyst -> editor -> auditor -> formatter.\n"
        f"\n"
        f"{context_mode_rule}\n"
        ...
        f"\n"
        f"### Step 5: Subagent runtime\n"
        f"\n"
        f"There is no Python brief-generation pipeline. Do NOT run `multi-agent-brief prepare`.\n"
        f"Use external subagents in this order:\n"
        f"\n"
        f"```text\n"
        f"Scout -> Screener -> Claim Ledger -> Analyst -> Editor -> Auditor -> Formatter\n"
        f"```\n"
        f"\n"
        f"### Step 6: Scout subagent\n"
        f"\n"
        f"Use the `scout` subagent to read approved sources and write `output/intermediate/candidate_claims.json`.\n"
        f"\n"
        f"### Step 7: Screener subagent\n"
        f"\n"
        f"Use the `screener` subagent to rank, dedupe, freshness-check, and write `output/intermediate/screened_candidates.json`.\n"
        f"\n"
        f"### Step 8: Claim Ledger subagent / validator\n"
        f"\n"
        f"Use the `claim-ledger` subagent or validator to create `output/intermediate/claim_ledger.json`.\n"
        f"Every claim must preserve source evidence and stable claim IDs.\n"
        f"\n"
        f"### Step 9: Analyst subagent\n"
        f"\n"
        f"Use the `analyst` subagent to write `output/intermediate/audited_brief.md` from `claim_ledger.json` and `user.md`.\n"
        ...
        f"\n"
        f"### Step 11: Final auditor subagent\n"
        f"\n"
        f"Use the `auditor` subagent to audit `output/intermediate/audited_brief.md` against `claim_ledger.json`.\n"
        f"This is the final delivery audit; there is no Python draft-level audit pass.\n"
        f"\n"
        f"### Step 12: Formatter / finalize tool\n"
        f"\n"
        f"Run `multi-agent-brief finalize --config ../mabw-workspace/config.yaml` so reader-facing Markdown/DOCX reflect the audited Markdown.\n"
        f"\n"
        f"### Step 13: Report artifacts\n"
        f"\n"
...
def render_docs(manifest: dict) -> dict[str, str]:
    ...
    harness = textwrap.dedent(f"""\
        ...
        ```text
        audit tool -> CompositeAuditAgent -> DeterministicAuditAgent -> QualityHarnessAuditAgent -> optional semantic audit adapter
        ```
...
def render_opencode_command_generate_brief(manifest: dict) -> str:
    ...
    return (
        ...
        "\n"
        "The Python CLI does not run the brief workflow. OpenCode subagents execute "
        "Scout/Screener/Claim Ledger/Analyst/Editor/Auditor/Formatter roles; Python "
        "commands provide setup, source planning, validation, audit, and finalize tools.\n"
        "\n"
        ...
        "does not exist or has not been merged, resolve sources before invoking Scout:\n"
