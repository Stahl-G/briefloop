...
OpenCode v2 的普通对话/生成适配与受限 Reviewer 是不同能力：v2 初始指令发现尚未验证遵守核查包白名单，因此 v2、未知版本及未安装时不会提供受限 Reviewer，事实核查在任务开始前说明原因。可以保留 OpenCode 主链，并在“独立审阅执行后端”选择 BriefLoop Agent。已验证 v1 路径保留；版本探针按可执行文件身份缓存，升级替换后重查，不随页面轮询反复启动。
