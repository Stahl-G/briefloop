...
function protocol(id:string){return id in acpArgs?'acp':id==='antigravity'?'antigravity-stream-json':id==='claude'?'claude-stream-json':id==='mimo'?'opencode-json':id==='codex'||id==='opencode'?'native-manager':null;}
function capabilities(id:string){const p=protocol(id);return {chat:!!p,cancel:!!p,resume:p==='acp'?'negotiated':p==='claude-stream-json'||p==='opencode-json'||p==='antigravity-stream-json',images:p==='acp'?'negotiated':p==='claude-stream-json',questions:p==='acp',steer:false,read_only:false,network_control:false,permission_modes:['runtime-native']};}
