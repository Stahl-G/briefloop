...
test('Pi RPC selects exact provider model, resumes its session and waits for agent_settled',async t=>{
 ...
 b.send(2,'start',{...f,runtime_id:'pi',execution_id:'pi',effort:'high',host_options:{mode:'none'},session_id:'/tmp/pi-test.jsonl',model:'test/flash',prompt:'hello',permission:'runtime-native',allow_web:null});assert.equal((await b.wait(x=>x.params?.kind==='end')).params.status,'completed');assert.equal(b.frames.filter(x=>x.params?.kind==='text').map(x=>x.params.text).join(''),'Pi OK');
