...
| Claude writer 命令 | 给写作者提供五动词入口 | 可选启用，一等 writer 路径 | `/mabw new`, `/mabw run`, `/mabw status`, `/mabw feedback`, `/mabw deliver` |
| Runtime handoff | 为外部 orchestrator 和 subagents 生成执行交接面 | 默认在场 | `multi-agent-brief run --workspace <workspace>` |
| 状态查看 | 查看当前 stage、blocker、artifact、计时 bucket 和下一步安全动作 | 默认在场 | `/mabw status`, `multi-agent-brief status` |
| 交付包 | 在 finalize 检查后输出读者可见 Markdown 和 DOCX | 默认在场 | `/mabw deliver`, `multi-agent-brief finalize`, `state finalize-complete` |
