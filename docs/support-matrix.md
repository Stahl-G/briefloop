...
| Experimental `solar-stock-periodic` ReportPack and schema-18 source plan (`briefloop new solar-stock-periodic --search-backend tavily`, Codex-kit preparation, fixed equity/event/theme universe, 20 independent Tavily tasks, per-task Extract outcomes, and fresh-only acquisition bundle) plus the `market-data` CLI channel (bounded no-redirect Yahoo chart-API adapter, manual-first `input/market_data/` intake, pre-provider active-invocation guard, one append-only snapshot per run and as-of date, deterministic comparison-table projection with explicit `NOT AVAILABLE` rows) | Experimental; source-discovery/report-contract and market-data acquisition surface only; no invented price/valuation fields; undated/out-of-window sources are background and cannot satisfy current-week high-priority coverage |
