...
| `briefloop run --workspace <path> --runtime dsh` | Experimental; DeepSeek Harness operator handoff launcher only — prints the DSH handoff protocol and never generates the brief or writes the ControlStore; fresh Store initialization stays Codex-bound |
...
| `briefloop runtime install --workspace <path> --runtime dsh` | Experimental; packaged DSH operating kit (eight role presets, operator skill, protocol reference) under `.dsh/` with exact hash-bound verification; comfort material only, never writes or re-binds the ControlStore |
