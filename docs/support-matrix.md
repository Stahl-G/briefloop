...
| Experimental `solar-stock-periodic` ReportPack and schema-19 source plan (`briefloop new solar-stock-periodic --core-ticker <TICKER> --search-backend tavily`, optional paired `--report-window-start/--report-window-end`, Codex-kit preparation, packaged peer/event/theme universe with a required explicit `--core-ticker`, 20 independent Tavily tasks, per-task Extract outcomes, and fresh-only acquisition bundle) plus the `market-data` CLI channel (strict `market_data_snapshot.v2`; profile-bound solar weekly XLSX, JSON/CSV, or bounded no-redirect Yahoo fill; manual cells win while product-owned formula outputs are recomputed; pre-provider active-invocation guard; adjusted history, FX, valuations, events, gaps/conflicts; two comparison tables, seven deterministic PNG charts, JSON projection and hash-bound manifest) | Experimental; source-discovery/report-contract and structured market-data surface only; required-series gaps/conflicts can block; no invented price/valuation/causality; workbook display objects are not evidence; undated/out-of-window news sources are background and cannot satisfy current-week high-priority coverage |
