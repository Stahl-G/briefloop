...
workspace source bytes so later support-sufficiency surfaces can consume
hashable evidence locations.
...
- Claim-Support Matrix behavior is available.
- support sufficiency has been decided.

It provides span-level traceability and archive reproducibility. Later
Claim-Support Matrix work is responsible for structured support rows and
blocking policy.
