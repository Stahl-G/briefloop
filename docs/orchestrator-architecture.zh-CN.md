...
`briefloop finalize` 会把 reader delivery bundle 写入 `output/delivery/`。配置 `source_appendix`，或旧配置请求 legacy `source_map` output format 时，来源附录会追加到交付 Markdown/DOCX 末尾，并同时保留 `output/source_appendix.md` 作为 audit/control copy。
...
v0.6.7 在运行交接单时创建 `output/intermediate/orchestrator_control_switchboard.json`。只有显式运行 `briefloop controls select` 时，才会写入 `output/intermediate/control_selections.json`。
...
- 只由 `briefloop provenance build` 创建。
