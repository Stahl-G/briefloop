...
局部更新与批量装配使用相同的摘录字段：引用给 `excerpt`，数字和日期给 `source_excerpt`。程序在冻结来源中精确定位；日期摘录转成规范日期记录，并保存相应引用。旧的仅含 `locator` 的记录仍可读取和更新，不因此宣称摘录已核查。错误最多一次反馈8项，整批失败不保存部分记录；新旧版本与重放约束不变。
...
`check_draft` 的 `writer_action.next_operation` 指明下一步：`repair_then_check` 只修列出的确定性问题；`submit_draft` 表示可提交当前工作稿，未核验项留给独立审阅。`needs_attention` 是诊断概况，不是新增的保存门禁。工具不支持的单位、数值出现位置待审阅及非强制目标字数，不要求为了消除提示改写正文；未核验不会标成通过。原有版本、来源身份与检查回执约束仍生效。
