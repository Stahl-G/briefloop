...
| `.codebuddy/agents/briefloop-*.md` | CodeBuddy project role sub-agents | Source-clone-only | Experimental native CodeBuddy role-agent assets for Scout, Claim Ledger, Analyst, Editor, Auditor, and Formatter. These are project-level sub-agents for handoff-assigned drafting, not gate, delivery, release, or control-file authority. |
