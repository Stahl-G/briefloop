...
The Orchestrator is the runtime main agent. It may run as the Hermes parent agent, Claude Code command context, Codex main agent, OpenCode primary agent, or operator fallback. Historical `manual` manifests are read-only; new and continuing runs must use an explicit canonical runtime.
...
workspace kit lets a runtime operate from the workspace without reading the BriefLoop
source repo.
