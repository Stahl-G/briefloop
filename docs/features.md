...
| Solar Stock Periodic workspace | Creates a fresh schema-19 experimental capital-markets weekly contract with a 20-task Tavily discovery plan | Experimental | `briefloop new solar-stock-periodic <workspace> --core-ticker <TICKER>` (the core issuer is a required Human input); JSON/CSV or profile-bound solar weekly XLSX can supply the separate frozen market-data snapshot |
...
| Solar Stock Periodic structured market data | Freezes manual-first JSON/CSV/XLSX or Yahoo-filled price, FX, valuation, event, gap, and conflict records; projects two tables plus seven deterministic charts | Experimental | Freeze the workbook period with `new ... --report-window-start <date> --report-window-end <date>`, then `briefloop market-data ingest --file <weekly.xlsx> --profile solar-weekly-v1`; product formulas are recomputed, embedded workbook charts are display-only, and market co-movement is not causal evidence |
...
| Solar market-data projection | `output/intermediate/market_data_tables.md`, `market_data_projection.json`, `market_data_chart_manifest.json`, and `output/charts/market_data/*.png` | Experimental |
