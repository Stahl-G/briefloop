...
后续薄包验收中，早一轮 0.20 产物通过原生 NSIS 安装，自动找到本机 Python 3.11.15；首次准备取消后未留下未完成的 venv，重试进入就绪状态。中文工作区使用确切服务 PID 启动，Ctrl+S 保存、退出重开、已有环境复用及 OpenCode + DeepSeek 官方 `deepseek-flash` 的真实文件写入与回读通过。该轮发现引用链接追加文字在 Word 丢失，随后安装源码 `2dbca6f237585da635642da4e8bef6ec501aff95` 的修复产物复测：安装器为 113,968,523 字节，SHA-256 `e75ce4c9164fb20772c8a08399db679ec8c6bb18e44af8b3712fd50270475fa1`；原生“另存为”的 Word 为 36,702 字节，SHA-256 `af9d8cb678f7e75da8e7385f13f21a3dbd2795c72ed7de44051df9e77bd37635`。Word 完整保留修复前后两段中英文校验文字，WPS 在 100% 缩放下确认文字和表格可见；设置页显示 App 0.20，检查更新时明确提示缺少 Windows 发行文件。该产物的引用末尾纯文本粘贴仍会继承链接样式，但不再丢弃文字；后续补丁 `2d176554` 已通过真实 Electron 剪贴板回归，未包含在上述安装器中。另一次基线命令行卸载成功移除应用，数据库哈希未变且 Word 保留；图形界面卸载启动受自动化工具限制，不计为通过。完整的受控原生更新下载与安装仍未验收。
