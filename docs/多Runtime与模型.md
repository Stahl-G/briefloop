...
检测到程序、可选择执行、协议握手成功和真实模型调用成功是不同状态。CLI 卡片的“可选择”只表示应用实现了该后端且找到本机程序；即使版本探测失败也不代表已完成账号或模型验证。安装列表中的其它 CLI 可能仅用于发现，不能执行。当前应用支持的执行后端仅为上表七个；Kilo、Kiro、Vibe 虽在底层 bridge 有 ACP 接线，尚未开放为应用执行后端。
