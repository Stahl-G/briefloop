...
这能防止一个常见的 agent 失败：简报看起来已经写完，但背后的流程证据并不完整。

健康状态下，一份简报不是因为 agent 说“我完成了”就算完成，而是因为 run state、artifacts、decisions 和 checks 对得上。
...
> MABW 能告诉你本期运行到哪了、缺什么、为什么能继续或不能继续。

## 2. 每个数字哪来的

MABW 会把事实 claim 和写作风格分开。

对于 claim、数字、日期、公司事实、政策、价格、产能、客户和项目进度，最重要的问题是：

> 这个东西从哪来的？

MABW 会记录 sources 和 claim support，让 reviewer 可以从最终句子一路追到来源材料和审计面。
...
> 在最终简报里随便指一个数字，问“这个数从哪来的？”系统应该能指回 claim、source、date 和 checks。

这是 MABW 最简单的企业级演示：打开一份成品，指着一个数字，追溯它。

## 3. 它学到了什么

MABW 可以保留读者偏好，但它不会偷偷学习口味。
...
被批准的 preference guidance 会进入 Improvement Ledger，被投影到 memory，并冻结进下一次 run 的 snapshot。这意味着：

- 未批准的建议不会影响未来 run；
- 被批准的 guidance 只影响后续 run，不会 retroactively 改变当前 run；
- 被 reverted 的 guidance 不会进入后续 snapshot；
- run manifest 会记录本次 materialized 了哪些 approved guidance。
...
> 如果你希望 MABW 记住一个写作偏好，它必须先经过你批准。之后你可以查看，也可以撤销。

## 4. 什么在替你把关

MABW 还会记录 guardrails：contracts、gates、policies 和 delivery checks。

这些不是读者偏好，而是交付标准。

例子：

- Required artifacts 必须存在，stage 才能算完成；
- missing source 或 stale source 可能阻断继续；
- source appendix 是交付 surface；
- repair plan 只结构化 feedback，不自动改稿；
- final output 不应向读者暴露 local paths 或 internal claim IDs。
