...
- 实验性 `solar-stock-periodic` 是 schema-19、fresh-only 的全球太阳能资本
  市场周报 ReportPack（核心标的为必填的 `--core-ticker` 输入）。运行时冻结 20 条独立 Tavily 发现任务（11 家上市公司、5 个
  ...
  支持 JSON/CSV 或 profile-bound 太阳能周报 XLSX；Excel 单元格优先，Yahoo 只补缺失
