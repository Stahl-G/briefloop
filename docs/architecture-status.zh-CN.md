...
- 实验性 Atomic Claim Graph 控制面可以校验可选
  `output/intermediate/atomic_claim_graph.json`，检查 whole-ledger coverage
  和确定性 Claim Ledger type consistency，暴露 Analyst/Editor no-new-atom
  contract boundary，并投影 atom ID reader residue。这只是结构可见性，不是
  evidence-span support sufficiency。
- 实验性 Evidence Span Registry 控制面可以校验可选
  ...
  Source Appendix span summary 与单独的 `output/source_appendix_trace.md`
  audit copy。这只是 span-level traceability 和 archive reproducibility，不是
  semantic support assessment 或 support-sufficiency gate。
...
- v0.9 support-sufficiency 后续结构，例如 Claim-Support Matrix、semantic
  support scoring 和 support-sufficiency gates
